...
const DASHBOARD_FRAME_IDS=new Set([
  ...
  'validatorCount','rewardChartTotal','sentimentScore'
]);
...
function renderMarketPrice(){
  ...
  if(change){
    const performance=Number.isFinite(Number(state.change))?Number(state.change):0;
    const formatted=`${performance>0?'+':''}${performance.toFixed(2)}%`;
    if(change.textContent!==formatted) change.textContent=formatted;
    change.dataset.rollText=formatted;
    change.dataset.rollValue=String(performance);
    change.classList.add('inj-live-change','market-performance');
    change.classList.toggle('up',performance>0);
    change.classList.toggle('down',performance<0);
    change.classList.toggle('flat',performance===0);
    change.setAttribute('aria-label',`Performance 24 ore ${performance.toFixed(2)} per cento`);
  }
}
...
(() => {
  ...
  function updateLiveDisplay(){
    // v8.9.5: unico renderer per il Live Display.
    // Evita l’alternanza legacy 4/6 decimali che causava contrazione e ritorno in loop.
    if(typeof window.renderLiveStatic === 'function') window.renderLiveStatic();
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    lockAllPercentages();
    updateLiveDisplay();
    setInterval(lockAllPercentages,1000);
  });
})();
...
(() => {
  const liveIds = new Set([
    ...
    'requiredInjPrice','rewardTargetStaked','rewardTargetDailyInj','rewardTargetApr'
  ]);
...
/* v8.1 — keep Live Display bottom metrics as stable one-line text */
(() => {
  ...
  rollValue = function(id, formatted, numericValue){
    if(stableLiveIds.has(id)){
      ...
      if(Number.isFinite(Number(numericValue))) el.dataset.rollValue=String(Number(numericValue));
      return;
    }
    return previousRollValue(id, formatted, numericValue);
  };
})();

/* v8.9 — definitive premium motion, stable typography and single card signal */
(() => {
  ...
  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeMarketChange','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards',
    'requiredInjPrice','rewardTargetStaked','rewardTargetDailyInj','rewardTargetApr'
  ]);

  function settleNumber(el, text){
    el.classList.add('rolling-number','rolling-number-v89');
    el.classList.remove('roll-active-v89','trend-up-v89','trend-down-v89');
    el.replaceChildren();
    for(const char of String(text)){
      const node=document.createElement('span');
      node.className=/\d/.test(char)?'roll-char-v89 roll-digit-v89':'roll-char-v89 roll-symbol-v89';
      node.textContent=char;
      el.appendChild(node);
    }
  }

  function stableLiveValue(el, text, numeric){
    if(el.dataset.rollText===text) return;
    el.classList.remove('rolling-number','rolling-number-v89','roll-active-v89','trend-up-v89','trend-down-v89');
    el.textContent=text;
    el.dataset.rollText=text;
    if(Number.isFinite(numeric)) el.dataset.rollValue=String(numeric);
  }
  ...
  window.rollValue = function(id, formatted, numericValue){
    ...
    const nextText=String(formatted);
    const nextValue=Number.isFinite(Number(numericValue))?Number(numericValue):parseNumber(nextText);

    if(LIVE_IDS.has(id)){
      stableLiveValue(el,nextText,nextValue);
      return;
    }
    ...
    clearTimeout(el._v89RollTimer);
    const comparable=previousText && !previousText.includes('—') && Number.isFinite(previousValue) && Number.isFinite(nextValue);
    const direction=comparable && nextValue<previousValue?'down':'up';
    const reserve=Math.max(Number(el.dataset.reserveChars||0),previousText.length,nextText.length);
    el.dataset.reserveChars=String(reserve);
    el.style.setProperty('--roll-reserve',`${Math.max(1,reserve)}ch`);
    el.classList.add('rolling-number','rolling-number-v89',`trend-${direction}-v89`);
    el.classList.remove(`trend-${direction==='up'?'down':'up'}-v89`);
    el.replaceChildren();

    const maxLength=Math.max(previousText.length,nextText.length);
    const oldPadded=previousText.padStart(maxLength,' ');
    const newPadded=nextText.padStart(maxLength,' ');
    for(let i=0;i<maxLength;i++){
      const oldChar=oldPadded[i];
      const newChar=newPadded[i];
      const isDigit=/\d/.test(newChar);
      const changed=comparable && isDigit && /\d/.test(oldChar) && oldChar!==newChar;
      if(changed){
        const slot=document.createElement('span');
        slot.className=`roll-slot-v89 roll-${direction}-v89`;
        const oldNode=document.createElement('span');
        oldNode.className='roll-old-v89'; oldNode.textContent=oldChar;
        const newNode=document.createElement('span');
        newNode.className='roll-new-v89'; newNode.textContent=newChar;
        slot.append(oldNode,newNode);
        el.appendChild(slot);
      }else{
        const node=document.createElement('span');
        node.className=isDigit?'roll-char-v89 roll-digit-v89':'roll-char-v89 roll-symbol-v89';
    ...
    el.dataset.rollText=nextText;
    if(Number.isFinite(nextValue)) el.dataset.rollValue=String(nextValue);
    requestAnimationFrame(()=>requestAnimationFrame(()=>el.classList.add('roll-active-v89')));
    el._v89RollTimer=setTimeout(()=>settleNumber(el,nextText),560);

    if(typeof window.signalDashboardCard==='function') window.signalDashboardCard(id,nextValue);
  };

  window.signalDashboardCard = function(id, numericValue){
    const el=getEl(id);
    const card=el?.closest('#dashboardView .card');
    const next=Number(numericValue);
    if(!card || !Number.isFinite(next)) return;
    const previous=Number(el.dataset.frameValueV89);
    el.dataset.frameValueV89=String(next);
    if(!Number.isFinite(previous) || Math.abs(next-previous)<Number.EPSILON) return;
    const direction=next>previous?'up':'down';
    card.classList.remove('live-data-up','live-data-down','premium-pulse-up-v89','premium-pulse-down-v89');
    void card.offsetWidth;
    card.classList.add(`premium-pulse-${direction}-v89`);
    clearTimeout(card._premiumPulseV89);
    card._premiumPulseV89=setTimeout(()=>card.classList.remove('premium-pulse-up-v89','premium-pulse-down-v89'),1050);
  };

  document.addEventListener('DOMContentLoaded',()=>{
    document.documentElement.classList.add('ui-v89');
    document.querySelectorAll('.rolling-number').forEach(el=>{
      if(el.closest('.landscape-live')) stableLiveValue(el,el.dataset.rollText||el.textContent,Number(el.dataset.rollValue));
    });
  },{once:true});
})();

/* v8.9.4 — isolated, non-animated Live Display renderer */
(() => {
  const LIVE_IDS = new Set([
    'landscapeMarketPrice','landscapeMarketChange','landscapeWalletWorth','landscapeWalletInj',
    'landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'
  ...
  const write = (id,text) => {
    ...
    if(el.dataset.liveStaticText===next && el.textContent===next) return;
    el.className = el.className
      .split(/\s+/)
      .filter(name => !/^roll|^trend-|^digit-change-|^value-glow-|^premium-pulse-/.test(name))
      .join(' ');
    el.replaceChildren(document.createTextNode(next));
    el.dataset.liveStaticText=next;
    el.dataset.rollText=next;
  };
  const money = (value,digits) => typeof displayMoney==='function'
    ? displayMoney(finite(value),digits)
    : `$${finite(value).toFixed(digits)}`;
  const injText = (value,digits=6) => finite(value).toLocaleString('it-IT',{
    minimumFractionDigits:digits,maximumFractionDigits:digits,useGrouping:true
  });

  function renderLiveStatic(){
    const panel=byId('landscapeLive');
    if(!panel || panel.hidden) return;
    const s=(typeof state!=='undefined'&&state)||window.state||{};
    const price=finite(s.price), available=finite(s.available), staked=finite(s.staked), rewards=finite(s.rewards);
    const total=available+staked+rewards;
    const change=finite(s.change);
    write('landscapeMarketPrice',money(price,4));
    write('landscapeWalletWorth',money(total*price,2));
    write('landscapeWalletInj',`${injText(total,6)} INJ`);
    write('landscapeWalletAvailable',injText(available,6));
    write('landscapeWalletStaked',injText(staked,6));
    write('landscapeWalletRewards',injText(rewards,6));
    const changeEl=byId('landscapeMarketChange');
    if(changeEl){
      write('landscapeMarketChange',`${change>0?'+':''}${change.toFixed(2)}% · 24H`);
      changeEl.classList.remove('positive','negative','neutral','up','down');
      changeEl.classList.add(change>0?'positive':change<0?'negative':'neutral');
    }
    panel.querySelectorAll('[class*="roll"],[class*="trend-"],[class*="value-glow"],[class*="premium-pulse"]').forEach(el=>{
      if(LIVE_IDS.has(el.id)) return;
      el.classList.remove('roll-active','roll-active-v89','trend-up-v89','trend-down-v89','value-glow-up','value-glow-down','premium-pulse-up-v89','premium-pulse-down-v89');
    });
  }

  const inheritedRoll=window.rollValue;
  window.rollValue=function(id,formatted,numericValue){
    if(LIVE_IDS.has(id)){
      write(id,formatted);
      const el=byId(id);
      if(el && Number.isFinite(Number(numericValue))) el.dataset.rollValue=String(Number(numericValue));
      return;
    }
    return inheritedRoll?.(id,formatted,numericValue);
  };

  let raf=0;
  function schedule(){
    if(raf) return;
    raf=requestAnimationFrame(()=>{ raf=0; renderLiveStatic(); });
  }
  window.renderLiveStatic=renderLiveStatic;
  document.addEventListener('DOMContentLoaded',()=>{
    const panel=byId('landscapeLive');
    if(!panel) return;
    panel.classList.add('live-static-v894');
    new MutationObserver(schedule).observe(panel,{attributes:true,attributeFilter:['hidden','aria-hidden']});
