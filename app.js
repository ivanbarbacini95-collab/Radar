...
async function loadMarket(){
  try{
    const ticker=await json('https://api.binance.com/api/v3/ticker/24hr?symbol=INJUSDT');
    ...
    // Metadata is optional and must never block dashboard startup.
    Promise.resolve().then(async()=>{
      let market=null;
      let source='Cached data';
      try{
        ...
        if(market?.market_cap) source='CoinGecko';
      }catch{}
      if(!market?.market_cap){
        try{
          const paprika=await json('https://api.coinpaprika.com/v1/tickers/inj-injective',6500);
          if(paprika?.quotes?.USD?.market_cap){
            market={market_cap:paprika.quotes.USD.market_cap,market_cap_rank:paprika.rank,circulating_supply:paprika.circulating_supply};
            source='CoinPaprika';
          }
        }catch{}
      }
      if(market?.market_cap){
        ...
        storage.setJSON('inj_market_metadata_v836',{marketCap:state.marketCap,marketRank:state.marketRank,circulatingSupply:state.circulatingSupply,source,time:Date.now()});
      }else{
        const cached=storage.getJSON('inj_market_metadata_v836',{});
      ...
      if(state.circulatingSupply>0) setText('circulatingSupplyValue',`${state.circulatingSupply.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`);
      render();
    }).catch(error=>console.warn('Optional market metadata unavailable',error));
    ...
    try{
      ...
      savePriceHistory();
      drawAll();
    }catch(error){ console.warn('Market history unavailable',error); }
  }catch(error){ console.warn(error); status('offline','Market unavailable'); }
}
...
(()=>{
  ...
  function diagnostics(){const out=document.getElementById('diagnosticOutput');const rows=[`Online: ${navigator.onLine?'Yes':'No'}`,`Service Worker: ${'serviceWorker'in navigator?'Supported':'No'}`,`Sthourge: ${(()=>{try{localStorage.setItem('_t','1');localStorage.removeItem('_t');return'OK'}catch{return'Error'}})()}`,`Canvas: ${!!document.createElement('canvas').getContext('2d')?'OK':'No'}`,`Wallet: ${state.address?'Loaded':'Not loaded'}`,`API: ${state.endpoint||'pending'}`];if(out)out.innerHTML=rows.join(' · ');toastV7('Diagnostics complete')}
  function bind(){
...
(() => {
  ...
  const sthourgeGet = (key, fallback='') => { try { return localStorage.getItem(key) ?? fallback; } catch { return fallback; } };
  const sthourgeSet = (key, value) => { try { localStorage.setItem(key, String(value)); } catch {} };
...
(() => {
  ...
  function bindStandaloneLiveMode(){
    ...
    button.addEventListener('click',event=>{
      event.preventDefault();
      event.stopImmediatePropagation();
      const saved=(localStorage.getItem('inj_address')||'').trim();
