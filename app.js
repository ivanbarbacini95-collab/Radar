...
const ROLLING_IDS=new Set([
  ...
  'ohlcOpen','ohlcHigh','ohlcLow','ohlcClose','ohlcVolume','marketChange','requiredInjPrice','rewardTargetSummary','rewardTargetStaked','rewardTargetDailyInj','rewardTargetApr',
...
function historyChange(ms){
  ...
  // Usa il punto più vicino al timeframe richiesto. Se lo storico locale è
  // ancora giovane, usa il primo punto disponibile invece di lasciare “—”.
  let base=rows[0];
  let bestDistance=Math.abs(base.t-target);
  for(const row of rows){
    const distance=Math.abs(row.t-target);
    if(distance<=bestDistance){ base=row; bestDistance=distance; }
    if(row.t>target && distance>bestDistance) break;
  }
  if(!base?.v || base.t===latest.t) base=rows[0];
  if(!base?.v || base.t===latest.t) return null;
  return {usd:latest.v-base.v,pct:((latest.v/base.v)-1)*100,baseTime:base.t,latestTime:latest.t,partial:rows[0].t>target};
}
...
function setPerformance(id,change){ const el=$(id); if(!el) return; el.textContent=formatPerformance(change); el.className=change?(change.usd>=0?'up':'down'):''; el.title=change?.partial?'Calcolato sullo storico disponibile più vicino al periodo richiesto':''; }
...
function renderRewardTarget(){
  const input=$('dailyRewardTargetUsd');
  const target=Math.max(0,number(input?.value));
  const dailyInj=number(state.staked)*(number(state.apr)/100)/365;
  setText('rewardTargetStaked',state.staked?`${inj(state.staked,4)} INJ`:'—');
  setText('rewardTargetDailyInj',dailyInj>0?`${inj(dailyInj,7)} INJ`:'—');
  setText('rewardTargetApr',state.apr?`${state.apr.toFixed(3)}%`:'—');
  if(!(target>0)){
    setText('requiredInjPrice','—');
    setText('rewardTargetSummary','Inserisci un obiettivo giornaliero superiore a $0');
    return;
  }
  if(!(dailyInj>0)){
    setText('requiredInjPrice','—');
    setText('rewardTargetSummary','Servono INJ in staking e un APR disponibile');
    return;
  }
  const requiredPrice=target/dailyInj;
  rollValue('requiredInjPrice',preciseMoney(requiredPrice,2),requiredPrice);
  const currentDailyUsd=dailyInj*number(state.price);
  setText('rewardTargetSummary',`Con ${inj(state.staked,2)} INJ in staking produci circa ${inj(dailyInj,6)} INJ/giorno (${money(currentDailyUsd)} al prezzo attuale).`);
}
...
document.addEventListener('input',event=>{
  if(event.target?.id==='dailyRewardTargetUsd'){
    storage.set('inj_daily_reward_target_usd',String(event.target.value||''));
    renderRewardTarget();
  }
});
...
/* v7.8.6 — stable layout, wallet-only compact mode, linear digit roll */
(() => {
  const id = value => document.getElementById(value);
  const THEME_ORDER=['black','light','ocean'];
  const THEME_META={
    black:{label:'Nero',icon:'●',themeColor:'#000000'},
    light:{label:'Chiaro',icon:'☀',themeColor:'#f5f8f9'},
    ocean:{label:'Blu scuro',icon:'◉',themeColor:'#06131d'}
  };
  let liveTimer=0;

  // Replace the previous single-layer flash with a true old/new vertical roll.
  if(typeof ROLLING_IDS!=='undefined'){
    ['landscapeMarketPrice','landscapeMarketChange','landscapeWalletWorth','landscapeWalletInj','landscapeWalletAvailable','landscapeWalletStaked','landscapeWalletRewards'].forEach(key=>ROLLING_IDS.add(key));
  }
  window.rollValue = rollValue = function(idValue,formatted,numericValue){
    const el=id(idValue); if(!el) return;
    const next=String(formatted ?? '');
    const previous=el.dataset.rollText ?? '';
    const oldNumber=Number(el.dataset.rollValue);
    const newNumber=Number(numericValue);
    if(previous===next){
      if(Number.isFinite(newNumber)) el.dataset.rollValue=String(newNumber);
      return;
    }
    const comparable=previous && !previous.includes('—') && previous.length===next.length && Number.isFinite(oldNumber) && Number.isFinite(newNumber);
    const direction=comparable && newNumber<oldNumber?'down':'up';
    const frag=document.createDocumentFragment();
    [...next].forEach((newChar,index)=>{
      const oldChar=previous[index] ?? '';
      const changed=comparable && /\d/.test(newChar) && /\d/.test(oldChar) && oldChar!==newChar;
      const slot=document.createElement('span');
      slot.className='digit-roll-slot';
      slot.dataset.kind=/\d/.test(newChar)?'digit':(/[.,]/.test(newChar)?'separator':(/[$€£¥]/.test(newChar)?'currency':'text'));
      if(changed){
        slot.classList.add(direction==='down'?'is-down':'is-up');
        const oldNode=document.createElement('span');oldNode.className='digit-roll-old';oldNode.textContent=oldChar;
        const newNode=document.createElement('span');newNode.className='digit-roll-new';newNode.textContent=newChar;
        slot.append(oldNode,newNode);
      }else{
        const stable=document.createElement('span');stable.className='digit-roll-stable';stable.textContent=newChar;slot.append(stable);
      }
      frag.append(slot);
    });
    el.classList.add('rolling-number-clean');
    el.replaceChildren(frag);
    el.dataset.rollText=next;
    if(Number.isFinite(newNumber)) el.dataset.rollValue=String(newNumber);
    clearTimeout(el._rollClean);
    el._rollClean=setTimeout(()=>{
      el.querySelectorAll('.digit-roll-slot').forEach(slot=>{
        const final=slot.querySelector('.digit-roll-new,.digit-roll-stable')?.textContent ?? '';
        slot.classList.remove('is-up','is-down');
        slot.replaceChildren();
        const stable=document.createElement('span');stable.className='digit-roll-stable';stable.textContent=final;slot.append(stable);
      });
    },260);
  };

  function setTheme(theme,persist=true){
    const next=THEME_ORDER.includes(theme)?theme:'black';
    document.body.classList.remove('light','ocean','theme-black');
    if(next==='light')document.body.classList.add('light');
    else if(next==='ocean')document.body.classList.add('ocean');
    else document.body.classList.add('theme-black');
    if(persist)localStorage.setItem('inj_theme',next);
    const button=id('drawerTheme'),meta=THEME_META[next];
    if(button){button.dataset.theme=next;button.querySelector('.interface-icon').textContent=meta.icon;button.querySelector('span:last-child').textContent=meta.label;button.classList.toggle('active',next!=='black')}
    let themeMeta=document.querySelector('meta[name="theme-color"]');
    if(themeMeta)themeMeta.content=meta.themeColor;
    requestAnimationFrame(()=>{try{drawAll?.()}catch{} window.dispatchEvent(new Event('resize'))});
  }
  function cycleTheme(event){event?.stopImmediatePropagation?.();const current=id('drawerTheme')?.dataset.theme||localStorage.getItem('inj_theme')||'black';setTheme(THEME_ORDER[(THEME_ORDER.indexOf(current)+1)%THEME_ORDER.length])}

  function syncCompactButton(){
    const active=document.body.classList.contains('compact-mode'),button=id('drawerCompactMode');
    button?.classList.toggle('active',active);button?.setAttribute('aria-pressed',active?'true':'false');
  }
  function toggleCompact(){
    const active=document.body.classList.toggle('compact-mode');localStorage.setItem('inj_compact_mode_v783',active?'on':'off');syncCompactButton();setTimeout(()=>window.dispatchEvent(new Event('resize')),50);
  }
  function syncOptionButton(buttonId,active,onLabel,offLabel){
    const button=id(buttonId);if(!button)return;
    button.classList.toggle('active',active);button.setAttribute('aria-pressed',active?'true':'false');
    const label=button.querySelector('span:last-child');if(label)label.textContent=active?onLabel:offLabel;
  }
  function toggleBodyOption(className,key,buttonId,onLabel,offLabel){
    const active=document.body.classList.toggle(className);localStorage.setItem(key,active?'on':'off');syncOptionButton(buttonId,active,onLabel,offLabel);setTimeout(()=>window.dispatchEvent(new Event('resize')),40);
  }


  function walletTotal(){return Number(state?.available||0)+Number(state?.staked||0)+Number(state?.rewards||0)}
  function syncLiveDisplay(){
    if(!id('landscapeLive')||id('landscapeLive').hidden)return;
    const price=Number(state?.price||0),total=walletTotal(),worth=total*price;
    setText('landscapeMarketPrice',typeof preciseMoney==='function'?preciseMoney(price,4):`$${price.toFixed(4)}`);
    setText('landscapeMarketChange',Number.isFinite(Number(state?.change))?`${Number(state.change)>=0?'+':''}${Number(state.change).toFixed(2)}%`:'—');
    setText('landscapeWalletWorth',typeof money==='function'?money(worth):`$${worth.toFixed(2)}`);
    setText('landscapeWalletInj',`${typeof inj==='function'?inj(total,4):total.toFixed(4)} INJ`);
    setText('landscapeWalletAvailable',typeof inj==='function'?inj(state?.available||0,4):Number(state?.available||0).toFixed(4));
    setText('landscapeWalletStaked',typeof inj==='function'?inj(state?.staked||0,4):Number(state?.staked||0).toFixed(4));
    setText('landscapeWalletRewards',typeof inj==='function'?inj(state?.rewards||0,4):Number(state?.rewards||0).toFixed(4));
    const address=String(state?.address||'');
    id('landscapeWalletAddress').textContent=address?`${address.slice(0,12)}…${address.slice(-8)}`:'Nessun wallet selezionato';
    id('landscapeWalletLabel').textContent='CONTROVALORE WALLET';
  }
  async function openLive(event){
    event?.stopImmediatePropagation?.();
    const panel=id('landscapeLive');if(!panel)return;
    panel.hidden=false;panel.setAttribute('aria-hidden','false');document.body.classList.add('live-display-open');
    id('drawerLandscapeLive')?.setAttribute('aria-pressed','true');syncLiveDisplay();clearInterval(liveTimer);liveTimer=setInterval(syncLiveDisplay,300);
    try{await panel.requestFullscreen?.()}catch{}
  }
  function closeLive(event){
    event?.stopImmediatePropagation?.();clearInterval(liveTimer);liveTimer=0;
    const panel=id('landscapeLive');if(panel){panel.hidden=true;panel.setAttribute('aria-hidden','true')}
    document.body.classList.remove('live-display-open');id('drawerLandscapeLive')?.setAttribute('aria-pressed','false');
    try{if(document.fullscreenElement)document.exitFullscreen?.()}catch{}
  }
  function openWalletManager(event){
    const button=event.target.closest?.('[data-drawer-action="manage-wallets"]');if(!button)return;
    event.stopImmediatePropagation();
    try{renderWalletManager?.()}catch{}
    id('mainDrawer')?.classList.remove('open');id('drawerBackdrop')?.classList.remove('visible');document.body.classList.remove('drawer-open');
    setTimeout(()=>id('walletManagerDialog')?.showModal(),80);
  }

  function init(){
    // Remove obsolete data-only state and recover from accidentally persisted compact layouts.
    document.body.classList.remove('data-mode');
    localStorage.removeItem('inj_data_mode_v56');
    // Capture phase neutralizes older listeners before they toggle obsolete modes.
    id('drawerTheme')?.addEventListener('click',cycleTheme,true);
    id('drawerCompactMode')?.addEventListener('click',event=>{event.stopImmediatePropagation();toggleCompact()},true);
    id('drawerHideCharts')?.addEventListener('click',event=>{event.stopImmediatePropagation();toggleBodyOption('hide-charts','inj_hide_charts_v786','drawerHideCharts','Grafici nascosti','Nascondi grafici')},true);
    id('drawerHideAnimations')?.addEventListener('click',event=>{event.stopImmediatePropagation();toggleBodyOption('animations-off','inj_animations_off_v786','drawerHideAnimations','Animazioni OFF','Animazioni')},true);
    id('drawerLargeNumbers')?.addEventListener('click',event=>{event.stopImmediatePropagation();toggleBodyOption('large-numbers','inj_large_numbers_v786','drawerLargeNumbers','Numeri grandi ON','Numeri grandi')},true);
    id('drawerLandscapeLive')?.addEventListener('click',openLive,true);
    id('landscapeLiveExit')?.addEventListener('click',closeLive,true);
    id('mainDrawer')?.addEventListener('click',openWalletManager,true);
    document.addEventListener('fullscreenchange',()=>{if(!document.fullscreenElement&&!id('landscapeLive')?.hidden)closeLive()});
    document.addEventListener('inj:wallet-changed',syncLiveDisplay);
    const stored=localStorage.getItem('inj_theme');setTheme(THEME_ORDER.includes(stored)?stored:(document.body.classList.contains('light')?'light':'black'),false);
    if(localStorage.getItem('inj_compact_mode_v783')==='on')document.body.classList.add('compact-mode');syncCompactButton();
    [['hide-charts','inj_hide_charts_v786','drawerHideCharts','Grafici nascosti','Nascondi grafici'],['animations-off','inj_animations_off_v786','drawerHideAnimations','Animazioni OFF','Animazioni'],['large-numbers','inj_large_numbers_v786','drawerLargeNumbers','Numeri grandi ON','Numeri grandi']].forEach(([cls,key,b,on,off])=>{const active=localStorage.getItem(key)==='on';document.body.classList.toggle(cls,active);syncOptionButton(b,active,on,off)});
    // Remove obsolete dashboard controls even if an older build recreates them.
    id('addWalletTab')?.remove();id('manageWalletsButton')?.remove();
    id('drawerDataMode')?.remove();document.body.classList.remove('data-mode');localStorage.removeItem('inj_data_mode_v56');
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();

window.addEventListener('DOMContentLoaded',()=>{ const input=$('dailyRewardTargetUsd'); const saved=storage.get('inj_daily_reward_target_usd'); if(input&&saved!==null&&saved!=='') input.value=saved; renderRewardTarget(); });
