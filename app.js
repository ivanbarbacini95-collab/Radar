...
/* v7.8.3 — menu wallet management, tri-theme, Compact Mode, themed Live Display, true digit roll */
(() => {
  ...
  window.rollValue = rollValue = function(idValue,formatted,numericValue){
    ...
    const next=String(formatted);
    const previous=el.dataset.rollText ?? el.textContent ?? '';
    ...
    for(let index=0;index<next.length;index++){
      ...
      const newChar=next[index];
      const digit=/\d/.test(newChar);
      const changed=comparable && digit && /\d/.test(oldChar) && oldChar!==newChar;
      const slot=document.createElement('span');
      slot.className=`roll-slot-v783${digit?'':' roll-symbol-slot'}${changed?` roll-${direction}-v783`:''}`;
      if(changed){
        const oldNode=document.createElement('span'); oldNode.className='roll-old-v783'; oldNode.textContent=oldChar;
        const newNode=document.createElement('span'); newNode.className='roll-new-v783'; newNode.textContent=newChar;
        slot.append(oldNode,newNode);
      }else{
        const stable=document.createElement('span'); stable.className='roll-new-v783'; stable.style.position='relative'; stable.textContent=newChar; slot.append(stable);
      }
      el.append(slot);
    }
    el.dataset.rollText=next;
    if(Number.isFinite(newNumber)) el.dataset.rollValue=String(newNumber);
    clearTimeout(el._roll783);
    el._roll783=setTimeout(()=>{
      [...el.querySelectorAll('.roll-slot-v783')].forEach(slot=>{
        const final=slot.querySelector('.roll-new-v783')?.textContent ?? '';
        slot.classList.remove('roll-up-v783','roll-down-v783');
        slot.replaceChildren();
        const stable=document.createElement('span'); stable.className='roll-new-v783'; stable.style.position='relative'; stable.textContent=final; slot.append(stable);
      });
    },470);
  };
