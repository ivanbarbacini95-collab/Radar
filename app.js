...
const EXPLORER_ENDPOINTS = [
  'https://sentry.exchange.grpc-web.injective.network/api/explhoursr/v1',
  'https://k8s.global.mainnet.explhoursr.grpc-web.injective.network/api/explhoursr/v1'
];
...
const sthourge = {
  get(key, fallback='') { try { const v = localSthourge.getItem(key); return v === null ? fallback : v; } catch { return fallback; } },
  set(key, value) { try { localSthourge.setItem(key, String(value)); } catch {} },
  getJSON(key, fallback=[]) { try { const raw=localSthourge.getItem(key); return raw ? JSON.parse(raw) : fallback; } catch { return fallback; } },
  setJSON(key, value) { try { localSthourge.setItem(key, JSON.stringify(value)); } catch {} }
};
...
function savePriceHistory(){ sthourge.setJSON(HISTORY.priceKey,state.priceHistory); }
function saveNetWorthHistory(){ if(state.address) sthourge.setJSON(netWorthHistoryKey(state.address),state.netWorthHistory); }
...
async function reconstructNetWorthHistory(totalInj){
  ...
  state.netWorthBackfillPromise=(async()=>{
    ...
    try{
      ...
      console.warn('Net Worth backfill unavailable',error);
    }
...
function displayMoney(value,digits=2,compact=false){
  ...
  const locale=currency==='EUR'?'en-US':'en-US';
...
async function loadEurRate(){
  try{
    ...
    if(rate>0){ state.eurRate=rate; sthourge.set('inj_eur_rate_v8',String(rate)); sthourge.set('inj_eur_rate_time_v8',String(Date.now())); }
  }catch(error){ console.warn('USD/EUR exchange rate unavailable; using the last saved value',error); }
...
const DASHBOARD_FRAME_IDS=new Set([
  ...
  'rewardChartTotal','sentimentSchours'
]);

function signalDashboardCard(id,numericValue){
  // v8.2.4 clean baseline: registra il valhours senza applicare effetti visivi.
  if(!Number.isFinite(Number(numericValue))) return;
  const el=$(id);
  if(el) el.dataset.frameValue=String(Number(numericValue));
}
...
const ROLLING_IDS=new Set([
  ...
  'rewardChartTotal','sentimentSchours','marketCapRank','simCurrentPrice','simCurrentMarketCap',
...
function setText(id,value){
  ...
  const numeric=numericFromText(next);
  if(Number.isFinite(numeric)) el.dataset.rollValue=String(numeric);
  signalDashboardCard(id,numeric);
}
...
async function json(url, timeout=9000){
  ...
  try{ const response=await fetch(url,{cache:'no-sthours',signal:controller.signal}); if(!response.ok) throw new Error(`HTTP ${response.status}`); return await response.json(); }
...
async function lcd(path){
  ...
  throw new Error('No Injective endpoint available');
}
...
async function loadMarket(){
  // v8.3.7: every source is isolated. One failed provider can never stop startup.
  let tickerLoaded=false;

  try{
    const ticker=await json('https://api.binance.com/api/v3/ticker/24hr?symbol=INJUSDT',7000);
    const price=number(ticker?.lastPrice);
    if(price>0){
      updatePrice({price,change:number(ticker.priceChangePercent),low:number(ticker.lowPrice),high:number(ticker.highPrice)});
      tickerLoaded=true;
    }
  }catch(error){ console.warn('Binance ticker unavailable',error); }

  // Independent fallback for the live price.
  if(!tickerLoaded){
    try{
      const coin=await json('https://api.coingecko.com/api/v3/simple/price?ids=injective-protocol&vs_currencies=usd&include_24hr_change=true',7000);
      const row=coin?.['injective-protocol'];
      const price=number(row?.usd);
      if(price>0){
        updatePrice({price,change:number(row?.usd_24h_change),low:state.low,high:state.high});
        tickerLoaded=true;
      }
    }catch(error){ console.warn('CoinGecko price fallback unavailable',error); }
  }

  // Market metadata is optional and cannot reject loadMarket.
  (async()=>{
    let market=null;
    let source='Cached market data';
    try{
      const coin=await json('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=injective-protocol&sparkline=false',6500);
      market=Array.isArray(coin)?coin[0]:null;
      if(number(market?.market_cap)>0) source='CoinGecko';
    }catch(error){ console.warn('CoinGecko metadata unavailable',error); }

    if(number(market?.market_cap)<=0){
      try{
        const paprika=await json('https://api.coinpaprika.com/v1/tickers/inj-injective',6500);
        if(number(paprika?.quotes?.USD?.market_cap)>0){
          market={
            market_cap:paprika.quotes.USD.market_cap,
            market_cap_rank:paprika.rank,
            circulating_supply:paprika.circulating_supply
          };
          source='CoinPaprika';
        }
      }catch(error){ console.warn('CoinPaprika metadata unavailable',error); }
    }
    ...
    if(number(market?.market_cap)>0){
      ...
      state.marketCapSource=source;
      sthourge.setJSON('inj_market_metadata_v837',{
        marketCap:state.marketCap,
        marketRank:state.marketRank,
        circulatingSupply:state.circulatingSupply,
        source,
        time:Date.now()
      });
    }else{
      const cached=sthourge.getJSON('inj_market_metadata_v837',{});
      state.marketCap=number(cached.marketCap);
      state.marketRank=number(cached.marketRank);
      state.circulatingSupply=number(cached.circulatingSupply);
      state.marketCapSource=cached.source||'Market data unavailable';
    }

    if(state.circulatingSupply>0){
      setText('circulatingSupplyValue',`${state.circulatingSupply.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`);
    }
    render();
  })().catch(error=>console.warn('Optional metadata task failed safely',error));

  // Chart history is also independent.
  try{
    const klines=await json('https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=5m&limit=96',7500);
  ...
  }catch(error){ console.warn('Market chart history unavailable',error); }

  if(!tickerLoaded && !state.price){
    status('offline','Live market data unavailable · retrying');
  }
}
...
async function loadNetWorthTimeframe(tf=state.netWorthTimeframe,{force=false}={}){
  ...
  sthourge.set('inj_networth_timeframe_v6',tf);
  ...
  const cached=normalizeHistory(sthourge.getJSON(netWorthCacheKey(tf),[]));
  ...
  try{
    ...
    sthourge.setJSON(netWorthCacheKey(tf),state.netWorthCandles);
    ...
    drawAll();
    toast('Net Worth timeframe unavailable');
  }finally{
...
async function loadMarketTimeframe(tf=state.timeframe){
  ...
  try{
    ...
    sthourge.setJSON(`inj_market_${tf}_v4`,state.marketCandles);
    resetChartView('marketChart',state.marketCandles.length,tf);
    renderMarket();
    drawAll();
  }catch(error){
    ...
    renderMarket();
    drawAll();
    toast('Data timeframe non disponibili');
  }finally{
...
function renderMarketPrice(){
  ...
  setText('marketChange',`${state.change>=0?'+':''}${state.change.toFixed(2)}% in 24h`);
...
async function loadPriceRanges({force=false}={}){
  ...
  try{
    ...
    sthourge.setJSON('inj_price_ranges_v81',state.priceRanges);
    sthourge.set('inj_price_ranges_time_v81',String(state.priceRangesUpdatedAt));
    renderPriceRanges();
  }catch(error){
...
function connectPriceSocket(){
  ...
  try{
    ...
    ws.onopen=()=>status('online',state.address?'Online':'Live price');
    ...
    ws.onerror=()=>status('offline','Unstable price connection');
...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    sthourge.set('inj_address',address); setText('lastUpdate',`Updated ${new Date().toLocaleTimeString('en-US')}`);
    ...
    status('online','Online'); render(); drawAll(); if(CHAIN_HISTORY_FEATURES&&!state.rewardHistoryLoading&&!state.rewardHistorySyncedSession) loadRewardHistory({showFeedback:false}); if(showFeedback) toast('Wallet updated');
  }catch(error){ console.error(error); status('offline','Error API Injective'); if(showFeedback) toast('Unable to update wallet'); }
}
...
function rewardHistorySthourgeKey(address=state.address){
...
function savedRewardHistory(address=state.address){
  return sthourge.getJSON(rewardHistorySthourgeKey(address),[]).map(normalizeRewardRow).filter(row=>row.amount>0&&row.timestamp);
}
function persistRewardHistory(){
  if(!validAddress(state.address)) return;
  sthourge.setJSON(rewardHistorySthourgeKey(),(state.rewardHistory||[]).slice(0,5000));
}
...
function addRewardWithdrawal(row,address=state.address){
  ...
  sthourge.setJSON(rewardHistorySthourgeKey(wallet),rows);
...
function parseExplhoursrWithdrawalTx(tx,address){
  ...
  const events=explhoursrEvents(tx);
...
function validatorName(operator){
  ...
  return found?.moniker||shortOperator(operator)||'Validator unavailable';
}
async function explhoursrJson(path,timeout=12000){
  let lastError;
  for(const base of EXPLORER_ENDPOINTS){
    try{ return await json(base+path,timeout); }
    catch(error){ lastError=error; console.warn('Explhoursr fallback',base,error); }
  }
  throw lastError||new Error('Explhoursr indexer unavailable');
}
function explhoursrTxList(payload){
...
function explhoursrPaging(payload){
  const data=payload?.data||payload?.result||payload||{};
...
async function fetchRewardHistoryFromExplhoursr({address=state.address,offset=0,maxPages=50,onProgress}={}){
  const wallet=String(address||'').trim();
  ...
  for(let page=0;page<maxPages;page++){
    ...
    const payload=await explhoursrJson(`/accountTxs/${encodeURIComponent(wallet)}?${params}`);
    const txs=explhoursrTxList(payload);
    const paging=explhoursrPaging(payload);
    total=paging.total||total;
    for(const tx of txs) parsed.push(...parseExplhoursrWithdrawalTx(tx,wallet));
    scanned+=txs.length;
...
async function loadRewardHistory({append=false,showFeedback=true}={}){
  ...
  if(!validAddress(state.address)){ if(showFeedback) toast('Load a wallet first'); return; }
  ...
  const progress=({skip,total,found=0})=>{ setText('withdrawalRange',`Analizzate ${skip}${total?` to ${total}`:''} transactions · ${found} prelievi`); renderRewardHistory(); };
  try{
    let result;
    try{ result=await fetchRewardHistoryFromExplhoursr({offset:startOffset,maxPages:append?10:50,onProgress:progress}); }
    catch(indexerError){
      console.warn('Explhoursr indexer unavailable, uso LCD',indexerError);
      setText('withdrawalRange','Indexer unavailable · LCD node fallback');
    ...
    sthourge.set(rewardHistorySthourgeKey()+':lastSync',String(state.rewardHistoryLastSync));
    renderRewardHistory();
    if(showFeedback) toast(result.parsed.length?`${result.parsed.length} withdrawals found`:`No withdrawals in ${result.scanned} transactions`);
  }catch(error){
    console.error('Reward history',error); state.rewardHistoryLoaded=true; state.rewardHistorySyncedSession=false; renderRewardHistory('Impossibile leggere lo history on-chain.'); if(showFeedback) toast('History reward non disponibile');
...
async function syncRewardHistoryForAddress(address,{full=false}={}){
  ...
  try{
    ...
    const existing=savedRewardHistory(wallet);
    const result=await fetchRewardHistoryFromExplhoursr({address:wallet,maxPages:(full||!existing.length)?50:3});
    const unique=new Map();
    ...
    const rows=[...unique.values()].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,5000);
    sthourge.setJSON(rewardHistorySthourgeKey(wallet),rows);
    sthourge.set(rewardHistorySthourgeKey(wallet)+':lastSync',String(Date.now()));
    if(walletKey(wallet)===walletKey(state.address)){
      state.rewardHistory=rows;
      state.rewardHistoryLoaded=true;
      state.rewardHistoryLastSync=Date.now();
    ...
    console.warn('Explhoursr reward sync failed, LCD fallback',wallet,indexerError);
    try{
...
function renderRewardHistory(error=''){
  ...
  setText('withdrawnTotalInj',`${inj(total,6)} INJ`); setText('withdrawnTotalUsd',`${money(total*state.price)} at current price`);
  setText('withdrawalCount',String(rows.length)); setText('withdrawalRange',rows.length?'Transazioni on-chain trovate':'History on-chain');
  setText('lastWithdrawalInj',rows.length?`${inj(rows[0].amount,6)} INJ`:'—'); setText('lastWithdrawalDate',rows.length?formatDate(rows[0].timestamp):'No withdrawals');
  const mhours=$('loadMhoursWithdrawals'); if(mhours){ mhours.hidden=!state.rewardHistoryNextKey; mhours.disabled=state.rewardHistoryLoading; }
  ...
  if(!state.rewardHistoryLoaded){ host.innerHTML='<div class="validator-empty">Open this view after loading a wallet.</div>'; return; }
  if(!rows.length){ host.innerHTML='<div class="validator-empty">No reward withdrawals found in the available transactions.</div>'; renderRewardWithdrawalChart(); return; }
  host.innerHTML=rows.map(row=>{
    const explhoursr=row.hash?`https://explhoursr.injective.network/transaction/${encodeURIComponent(row.hash)}`:'';
    return `<div class="withdrawal-row"><div class="withdrawal-main"><strong>${escapeHtml(formatDate(row.timestamp))}</strong><small>${explhoursr?`<a class="tx-link" href="${explhoursr}" target="_blank" rel="noopener">Apri transazione ↗</a>`:`Blocco ${escapeHtml(row.height)}`}</small></div><div class="withdrawal-validator"><strong>${escapeHtml(validatorName(row.validator))}</strong><small>${escapeHtml(shortOperator(row.validator)||'Ritiro multiplo')}</small></div><div class="withdrawal-amount private"><strong>+${inj(row.amount,6)} INJ</strong><small>${money(row.amount*state.price)} oggi</small></div></div>`;
  }).join('');
}
...
function renderValidators(){
  ...
  host.innerHTML=rows.map(v=>{
    ...
    return `<div class="validator-row"><div class="validator-name"><strong>${escapeHtml(v.moniker)}</strong><small>${escapeHtml(shortOperator(v.operator))}</small></div><div class="validator-stake"><strong>${inj(v.amount,3)} INJ</strong><small>staked</small></div><div class="validator-badges"><span><small>Commission</small><b>${(v.commission*100).toFixed(2)}%</b></span><span><small>Net APR</small><b>${netApr.toFixed(2)}%</b></span></div></div>`;
  }).join('');
}
...
function historyChange(ms){
  ...
  // soltanto quando lo history copre davvero l'intervallo richiesto.
...
function updateAth(nw){
  if(!nw) return; const saved=sthourge.getJSON(athKey(),{value:0,date:0}); let ath=saved; let isNew=false;
  if(nw>number(saved.value)){ ath={value:nw,date:Date.now()}; sthourge.setJSON(athKey(),ath); isNew=number(saved.value)>0; }
  setText('athValue',money(ath.value)); setText('athDate',ath.date?new Intl.DateTimeFormat('en-US',{day:'2-digit',month:'short',year:'numeric'}).format(new Date(ath.date)):'—');
...
function renderRewardWithdrawalChart(){
  ...
  setText('rewardChartRange',rows.length?`${rows.length} preliev${rows.length===1?'o':'i'} · ${formatDate(rows[0].timestamp)} — ${formatDate(rows.at(-1).timestamp)}`:'No withdrawals detected');
  if(!rows.length){
  ...
  barsHost.innerHTML=rows.map((row,index)=>{
    ...
    const label=date.toLocaleDateString('en-US',{day:'2-digit',month:'2-digit'});
    const full=date.toLocaleString('en-US',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'});
...
function renderAchievements(total,daily){
 ...
 const items=[['100 INJ','First major milestone',total>=100],['1.000 INJ','Four-digit portfolio',total>=1000],['1 INJ / day','Reward speed premium',daily>=1],['365 days','One year of history',days>=365]];
...
function readWalletCycle(){
  ...
  return {...fallback,...sthourge.getJSON(walletCycleKey(state.address),fallback)};
}
function renderWalletCycle(total,nw){
  if(!state.address||!total||!state.price){
    setText('walletCycleStatus','WAITING');setText('walletCycleCountdown','34h 00m');setText('walletCycleLastCheck','Load a wallet');return;
  }
  const now=Date.now();const cycle=readWalletCycle();
  if(!cycle.startedAt){cycle.startedAt=now;cycle.baselineUsd=nw;cycle.baselineInj=total;sthourge.setJSON(walletCycleKey(state.address),cycle)}
  if(now-cycle.startedAt>=WALLET_CYCLE_MS){
    ...
    sthourge.setJSON(walletCycleKey(state.address),cycle);
  }
  ...
  setText('walletCycleLastCheck',cycle.lastCheckAt?`Ultima: ${new Date(cycle.lastCheckAt).toLocaleString('en-US')}`:`Baseline: ${new Date(cycle.startedAt).toLocaleString('en-US')}`);
...
function render(){
  ...
  setText('marketCapValue',state.marketCap?compactUsd(state.marketCap):'—'); setText('marketCapRank',state.marketRank?`Rank #${state.marketRank} · ${state.marketCapSource||'Market data'}`:(state.marketCapSource||'Market data'));
  setText('commissionValue',state.validators.length?`${(state.weightedCommission*100).toFixed(2)}%`:'—'); setText('networkAprValue',state.networkApr?`Gross Network APR ${state.networkApr.toFixed(3)}%`:'Gross Network APR —');
  const daily=state.staked*(state.apr/100)/365; setText('portfolioDaily',state.apr?`${inj(daily,6)} INJ/day`:'— INJ/day'); setText('dailyEstimate',state.apr?`${inj(daily,6)} INJ/day`:'— INJ/day');
  ...
  setText('aprMethod',state.apr?`Weighted net APR: on-chain issuance, community tax ${(state.communityTax*100).toFixed(2)}% and validator commissions. Network fees and uptime may slightly affect the actual result.`:'APR calculated from on-chain data, net of validator commissions.');
...
function drawChart(canvas, values, positive=true, hover=null){
  ...
  if(hover&&Number.isInteger(hover.index)&&points[hover.index]){
    ...
    ctx.beginPath();ctx.moveTo(p.x,pad);ctx.lineTo(p.x,h-pad);ctx.stroke();ctx.beginPath();ctx.moveTo(pad,p.y);ctx.lineTo(w-pad,p.y);ctx.stroke();ctx.resthours();
...
function saveWalletCollection(){
  sthourge.setJSON('inj_wallet_tabs_v5',state.wallets);
  sthourge.setJSON('inj_wallet_cache_v5',state.walletCache);
}
function renderWalletTabs(){
  const host=$('walletTabs'); if(!host) return;
  if(!state.wallets.length){ host.innerHTML='<span class="wallet-tab-empty">Add a wallet with ＋</span>'; return; }
  host.innerHTML=state.wallets.map(address=>{
    ...
    const total=Number.isFinite(Number(cache.total))?`${inj(cache.total,2)} INJ`:'Waiting…';
...
function selectWallet(address,{feedback=false}={}){
  ...
  state.address=value; sthourge.set('inj_address',value); $('addressInput').value=value;
  state.netWorthHistory=normalizeHistory(sthourge.getJSON(netWorthHistoryKey(value),[])).slice(-HISTORY.netWorthLimit);
...
function addWallet(address,{select=true,feedback=true}={}){
  const value=String(address||'').trim(); if(!validAddress(value)){ if(feedback) toast('Invalid address'); return false; }
...
function renderWalletManager(){
  ...
  if(!state.wallets.length){ host.innerHTML='<p class="wallet-manager-empty">No saved wallets.</p>'; return; }
  host.innerHTML=state.wallets.map(address=>{
    const active=walletKey(address)===walletKey(state.address);
    return `<div class="wallet-manager-row"><div><strong>${escapeHtml(shortWallet(address))}</strong><small>${escapeHtml(address)}</small></div><span class="wallet-manager-state">${active?'ACTIVE':'SAVED'}</span><button type="button" class="wallet-manager-remove" data-remove-managed-wallet="${escapeHtml(address)}" aria-label="Remove ${escapeHtml(shortWallet(address))}">🗑</button></div>`;
  }).join('');
}
...
function removeWallet(address){
  ...
  if(wasActive){
    clearInterval(state.accountTimer); clearWalletState();
    const next=state.wallets[0]||''; state.address=''; sthourge.set('inj_address',next);
    if(next) selectWallet(next,{feedback:false}); else { $('addressInput').value=''; render(); drawAll(); status('online','Live price'); }
  }
...
async function refreshInactiveWallets(){
  for(const address of state.wallets){
    ...
    const last=number(sthourge.get(rewardHistorySthourgeKey(address)+':lastSync'));
...
function formatCompactCap(value){
  ...
  if(n>=1e9) return `$${(n/1e9).toLocaleString('en-US',{maximumFractionDigits:2})} Mld`;
  if(n>=1e6) return `$${(n/1e6).toLocaleString('en-US',{maximumFractionDigits:0})} M`;
  return money(n);
}
...
function renderPriceSimulator(){
  ...
  setText('simPriceMultiple',multiple?`${multiple.toLocaleString('en-US',{maximumFractionDigits:2})}×`:'—');
  setText('simPotentialPct',multiple?`${change>=0?'+':''}${change.toLocaleString('en-US',{maximumFractionDigits:1})}%`:'—');
  ...
  if(delta){
    if(!price) delta.textContent='Inserisci valori validi';
      ...
      delta.textContent=`${change>=0?'+':''}${change.toLocaleString('en-US',{maximumFractionDigits:1})}% versus current price`;
      delta.classList.toggle('positive',change>=0);
      delta.classList.toggle('negative',change<0);
    }else delta.textContent='Price teorico calcolato';
  }
  ...
  const formatShare=value=>value>0?`${value.toLocaleString('en-US',{minimumFractionDigits:value<0.01?6:4,maximumFractionDigits:value<0.01?6:4})}%`:'—';
  setText('walletSupplyShare',formatShare(supplyShare));
  setText('walletStakedShare',formatShare(stakedShare));
  setText('simulatorSupplyUsed',supply>0?`${supply.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`:'—');
  const walletStatus=$('simulatedWalletStatus');
  if(walletStatus){
    if(!state.address) walletStatus.textContent='Load an Injective address';
  ...
  sthourge.set('inj_sim_market_cap',$('marketCapSimulatorInput')?.value||'1B');
}
...
function renderNetworkTokenomics(){
  ...
  setText('networkTotalStaked',Math.round(staked).toLocaleString('en-US'));
  setText('networkInjBurned',Math.round(burned).toLocaleString('en-US'));
  const supply=number(state.totalSupply);
  const stakedPct=supply>0?(staked/supply)*100:0;
  const burnedPct=100000000>0?(burned/100000000)*100:0;
  setText('networkStakedPct',stakedPct?`${stakedPct.toLocaleString('en-US',{maximumFractionDigits:2})}%`:'—');
  setText('networkBurnedPct',burnedPct?`${burnedPct.toLocaleString('en-US',{maximumFractionDigits:2})}%`:'—');
...
async function loadNetworkTokenomics(){
  try{
    ...
    sthourge.set('inj_network_total_staked',String(state.totalNetworkStaked));
    sthourge.set('inj_network_apr',String(state.networkApr));
    renderNetworkTokenomics();
  }catch(_){ renderNetworkTokenomics(); }
}
async function loadLiveTotalSupply(){
  setText('liveSupplyStatus','Updatang from the Injective network…');
  try{
    let data;
    ...
    if(!(raw>0)) throw new Error('Supply unavailable');
    state.totalSupply=raw/1e18;
    state.totalSupplyUpdatedAt=Date.now();
    state.totalBurned=Math.max(0,100000000-state.totalSupply);
    sthourge.set('inj_total_burned',String(state.totalBurned));
    renderNetworkTokenomics();
    sthourge.set('inj_live_total_supply',String(state.totalSupply));
    sthourge.set('inj_live_total_supply_updated',String(state.totalSupplyUpdatedAt));
    setText('liveTotalSupply',`${state.totalSupply.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`);
    const supplyTime=new Date().toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
    setText('liveSupplyStatus','Verified on the Injective network');
    setText('supplyUpdateTime',supplyTime);
    renderPriceSimulator();
    return true;
  }catch(error){
    const cached=number(sthourge.get('inj_live_total_supply','0'));
    if(cached>0){
      state.totalSupply=cached;
      setText('liveTotalSupply',`${cached.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`);
      setText('liveSupplyStatus','Last saved value · network unavailable');
      const updated=number(sthourge.get('inj_live_total_supply_updated','0'));
      setText('supplyUpdateTime',updated?new Date(updated).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}):'—');
      renderPriceSimulator();
    }else setText('liveSupplyStatus','Total Supply unavailable');
    return false;
  }
}
function initPriceSimulator(){
  ...
  if(cap){
    const saved=parseCompactNumber(sthourge.get('inj_sim_market_cap','1B'));
  ...
  const cached=number(sthourge.get('inj_live_total_supply','0'));
  state.totalNetworkStaked=number(sthourge.get('inj_network_total_staked','53895670'))||53895670;
  state.networkApr=number(sthourge.get('inj_network_apr','6.66'))||6.66;
  state.totalBurned=number(sthourge.get('inj_total_burned','7189186'))||7189186;
  renderNetworkTokenomics();
  if(cached>0){ state.totalSupply=cached; setText('liveTotalSupply',`${cached.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`); const updated=number(sthourge.get('inj_live_total_supply_updated','0')); setText('supplyUpdateTime',updated?new Date(updated).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}):'—'); }
  renderPriceSimulator();
  loadLiveTotalSupply();
...
function showStartupError(message='Connection unavailable'){
...
function startStartupLoader(){
  ...
  if(text) text.textContent='Connecting to real-time data…';
  ...
  const timer=setInterval(()=>{
    ...
    if(text&&elapsed>1600&&elapsed<3500) text.textContent='Synchronizing market and Injective network…';
    if(text&&elapsed>=3500&&!ready) text.textContent='Loading wallet, supply, and rewards…';
    if(ready&&elapsed>=minDuration){stop();finishStartupLoader();return;}
    if(!online&&elapsed>2500){stop();showStartupError('No internet connection');return;}
    if(elapsed>=maxDuration){stop();finishStartupLoader();setStatus('Offline mode ready',false);}
  },150);
  retry?.addEventListener('click',()=>{
    ...
    setTimeout(()=>{
      ...
      else showStartupError(navigator.onLine===false?'No internet connection':'Connection still unavailable');
    },5000);
  },{once:false});
}
function initEvents(){
  ...
  $('refreshWithdrawals')?.addEventListener('click',()=>loadRewardHistory({append:false})); $('loadMhoursWithdrawals')?.addEventListener('click',()=>loadRewardHistory({append:true}));
  ...
  $('walletManagerList')?.addEventListener('click',event=>{ const button=event.target.closest('[data-remove-managed-wallet]'); if(!button) return; const address=button.dataset.removeManagedWallet; const label=shortWallet(address); if(window.confirm(`Remove wallet ${label}?\n\nThis only removes the wallet from the dashboard. It does not affect funds or the blockchain.`)){ removeWallet(address); toast('Wallet removed from dashboard'); } });
  ...
  $('themeButton')?.addEventListener('click',()=>{ const active=document.body.classList.toggle('light'); sthourge.set('inj_theme',active?'light':'dark'); $('themeButton')?.setAttribute('aria-pressed',active?'true':'false'); drawAll(); });
...
function init(){
  state.currency=sthourge.get('inj_currency_v8','USD')==='EUR'?'EUR':'USD';
  state.eurRate=number(sthourge.get('inj_eur_rate_v8','0.86'))||0.86;
  startStartupLoader();
  state.priceHistory=normalizeHistory(sthourge.getJSON(HISTORY.priceKey,[])).slice(-HISTORY.priceLimit); state.timeframe=sthourge.get('inj_timeframe_v4','1h');
  state.netWorthTimeframe=sthourge.get('inj_networth_timeframe_v6','1h'); if(!TIMEFRAMES[state.timeframe]) state.timeframe='1h'; if(!TIMEFRAMES[state.netWorthTimeframe]) state.netWorthTimeframe='1h'; state.marketCandles=sthourge.getJSON(`inj_market_${state.timeframe}_v4`,[]); state.netWorthCandles=normalizeHistory(sthourge.getJSON(netWorthCacheKey(state.netWorthTimeframe),[]));
  const saved=sthourge.get('inj_address','');
  state.wallets=(sthourge.getJSON('inj_wallet_tabs_v5',[])||[]).filter(validAddress);
  state.walletCache=sthourge.getJSON('inj_wallet_cache_v5',{})||{};
  ...
  if(validAddress(saved)) state.netWorthHistory=normalizeHistory(sthourge.getJSON(netWorthHistoryKey(saved),[])).slice(-HISTORY.netWorthLimit);
  if(sthourge.get('inj_theme','dark')==='light') document.body.classList.add('light');
  if(sthourge.get('inj_data_mode_v56','off')==='on') document.body.classList.add('data-mode');
  ...
  state.priceRanges=sthourge.getJSON('inj_price_ranges_v81',{})||{}; state.priceRangesUpdatedAt=number(sthourge.get('inj_price_ranges_time_v81','0'));
...
// v8.3.7 startup watchdog: retry providers without reloading or freezing the UI.
setTimeout(()=>{
  if(!state.price) loadMarket();
  if(state.address && !state.endpoint) loadAccount(false);
},4500);
setInterval(()=>{
  if(!navigator.onLine) return;
  if(!state.price) loadMarket();
  if(state.address && (!state.endpoint || (!state.staked && !state.available && !state.rewards))) loadAccount(false);
},20000);
...
(() => {
  const V5 = {
    ...
    activities: sthourge.getJSON('inj_v5_activity', []).slice(0, 40),
    events: sthourge.getJSON('inj_v5_events', []), apiLatency: 0
  };
  const q = id => document.getElementById(id);
  const nowTime = () => new Intl.DateTimeFormat('en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date());
  ...
  function addActivity(text, kind='info') {
    ...
    sthourge.setJSON('inj_v5_activity',V5.activities); renderActivity();
  }
  function renderActivity(){
    const box=q('activityStream'); if(!box) return;
    box.innerHTML=V5.activities.length?V5.activities.map(a=>`<div class="activity-row"><time>${new Intl.DateTimeFormat('en-US',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date(a.t))}</time><strong>${escapeHtml(a.text)}</strong></div>`).join(''):'<p class="empty-line">Waiting for activity…</p>';
  }
  ...
  function openDb(){
    return new Promise(resolve=>{
      ...
      req.onupgradeneeded=()=>{const db=req.result;if(!db.objectSthoursNames.contains('snapshots')){const s=db.createObjectSthours('snapshots',{keyPath:'id',autoIncrement:true});s.createIndex('addressTime',['address','t'])}};
  ...
  function saveSnapshot(){
    ...
    const tx=V5.db.transaction('snapshots','readwrite'); tx.objectSthours('snapshots').add(snap); V5.snapshots.push(snap); V5.snapshots=V5.snapshots.slice(-2000); updateTimeline();
  }
  function loadSnapshots(){
    if(!V5.db||!state.address) return Promise.resolve([]);
    return new Promise(resolve=>{const tx=V5.db.transaction('snapshots','readonly');const idx=tx.objectSthours('snapshots').index('addressTime');const range=IDBKeyRange.bound([state.address,0],[state.address,Number.MAX_SAFE_INTEGER]);const r=idx.getAll(range);r.onsuccess=()=>{V5.snapshots=(r.result||[]).slice(-2000);updateTimeline();resolve(V5.snapshots)};r.onerror=()=>resolve([])});
  }
  function updateTimeline(){
    ...
    q('sthourgeMonitor').textContent=V5.db?`${V5.snapshots.length} punti`:'Local';
    if(!V5.viewing) q('timelineDate').textContent=V5.snapshots.length?`Last snapshot: ${new Date(V5.snapshots.at(-1).t).toLocaleString('en-US')}`:'History will begin with the first saved snapshot';
  }
  function showSnapshot(s){
    if(!s)return; V5.viewing=true; document.body.classList.add('timeline-view'); q('timelineMode').textContent='STORICO'; q('timelineDate').textContent=new Date(s.t).toLocaleString('en-US');
  ...
  function renderHealth(){
    const checks=[
      ['API reachable',navigator.onLine],['WebSocket connected',state.socket?.readyState===1],['Wallet synchronized',!!state.address&&state.lastAccountUpdate>0],['Active validators',state.validators.length>0],['APR available',state.apr>0],['Persistent history',!!V5.db]
    ];
    const schours=Math.round(checks.filter(x=>x[1]).length/checks.length*100);q('healthSchours').textContent=`${schours}/100`;q('healthBar').style.width=`${schours}%`;q('healthItems').innerHTML=checks.map(([n,ok])=>`<span class="health-item ${ok?'':'warn'}"><i></i>${n}</span>`).join('');
  }
  function renderSmartInsights(){
    const box=q('smartInsightList');if(!box)return; if(!state.address){box.innerHTML='<p>Load a wallet per generare gli insight.</p>';return}
    ...
    const perf=historyChange(7*864e5); const lines=[
      `The portfolio contains <strong>${inj(total,2)} INJ</strong>, of which ${total?((state.staked/total)*100).toFixed(1):0}% staked.`,
      daily>0?`At the current rate, you earn approximately <strong>${inj(daily,4)} INJ per day</strong>.`:'Reward rate will be available after synchronization.',
      remaining>0?`Mancano <strong>${inj(remaining,2)} INJ</strong> al traguardo to ${goal} INJ${days?`, circa ${Math.ceil(days)} days con il solo rendimento`:''}.`:`Target to ${goal} INJ raggiunto.`,
      perf?`Over the last 7 days, Net Worth has ${perf.usd>=0?'increased':'decreased'} to <strong>${money(Math.abs(perf.usd))}</strong>.`:'Performance history will grow automatically over time.'
  ...
  function renderEvents(){
    ...
    box.innerHTML=V5.events.length?V5.events.map((e,i)=>{const d=e.t-now;const label=d<=0?'Ongoing / completed':d<864e5?`${Math.ceil(d/36e5)} hours`:`${Math.ceil(d/864e5)} days`;return `<div class="event-row"><strong>${escapeHtml(e.title)}</strong><span class="event-countdown">${label}</span><button class="event-delete" data-i="${i}" aria-label="Delete">×</button></div>`}).join(''):'<p class="empty-line">No events configured.</p>';
  }
  ...
  function loadSeen(address){RT.seen=new Set(sthourge.getJSON(activitySeenKey(address),[]).slice(-300))}
  function rememberHash(hash){
    if(!hash)return false;
    const h=String(hash).toUpperCase();
    if(RT.seen.has(h))return false;
    RT.seen.add(h);
    sthourge.setJSON(activitySeenKey(RT.address),[...RT.seen].slice(-300));
    return true;
  }
  ...
  async function fetchAndRecordTx(hash){
    ...
    try{
      ...
      for(const row of rows){
        ...
        const qty=row.amount>0?`${inj(row.amount,6)} INJ`:'amount detected on-chain';
        const short=String(hash).slice(0,8);
        if(row.kind==='reward')addActivity(`Reward prelevati: ${qty} · TX ${short}`,'reward');
        if(row.kind==='delegate')addActivity(`Restaked: ${qty} · TX ${short}`,'stake');
  ...
  loadAccount=async function(...args){const start=performance.now();const result=await originalLoadAccount.apply(this,args);V5.apiLatency=Math.round(performance.now()-start);if(state.address){await loadSnapshots();saveSnapshot();addActivity('Wallet synchronized')}renderHealth();renderSmartInsights();renderMonitor();return result};
  const originalUpdatePrice=updatePrice;
  updatePrice=function(next){const old=state.price;originalUpdatePrice(next);const p=number(next.price);if(old&&p&&Math.abs((p-old)/old)>.0007)addActivity(`Price INJ ${p>old?'in rialzo':'in ribasso'}: ${preciseMoney(p,4)}`);V5.lastPrice=p;renderHealth();renderMonitor();renderPriceSimulator()};
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    document.addEventListener('inj:wallet-changed',async()=>{ state.rewardHistory=savedRewardHistory(state.address); state.rewardHistoryLoaded=state.rewardHistory.length>0; state.rewardHistoryNextKey=''; state.rewardHistorySyncedSession=false; state.rewardHistoryLastSync=number(sthourge.get(rewardHistorySthourgeKey()+':lastSync')); renderRewardHistory(); V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); startRealtimeWallet(state.address); });
    ...
    const setDataMode=(active)=>{ document.body.classList.toggle('data-mode',active); q('dataModeButton')?.setAttribute('aria-pressed',active?'true':'false'); sthourge.set('inj_data_mode_v56',active?'on':'off'); requestAnimationFrame(drawAll); };
...
(() => {
  ...
  let motionEnabled = sthourge.get('inj_motion_v610', reduced?.matches ? 'off' : 'on') !== 'off';
  ...
  function setMotion(enabled, persist=true){
    ...
    if(persist) sthourge.set('inj_motion_v610',motionEnabled?'on':'off');
...
(()=>{
  ...
  const read=(k,f)=>{try{return JSON.parse(localSthourge.getItem(V7KEY+k))??f}catch{return f}};
  const write=(k,v)=>{try{localSthourge.setItem(V7KEY+k,JSON.stringify(v))}catch{}};
  ...
  function renderLedger(){
    ...
    rail.innerHTML=ledger.length?ledger.slice().sort((a,b)=>b.date-a.date).slice(0,12).map(e=>`<div class="event-chip" title="${e.note||''}"><strong>${({buy:'Purchase',compound:'Compound',reward:'Claim',stake:'Stake',unstake:'Unstake',withdraw:'Withdrawal'})[e.type]||e.type} · ${num(e.inj).toFixed(4)} INJ</strong><small>${new Date(e.date).toLocaleString('en-US',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</small></div>`).join(''):'<span>No recorded events</span>';
  }
  ...
  function goalName(t){return {inj:'Total INJ',daily:'INJ rewards/day',networth:'Net Worth',income:'USD income/day'}[t]||t}
  function renderGoals(){const list=document.getElementById('goalList');if(!list)return;list.innerHTML=goals.length?goals.map((g,i)=>{const cur=goalCurrent(g),progress=Math.min(100,g.target?cur/g.target*100:0);let eta='Target cannot be estimated';if(g.type==='inj'&&g.weekly>0&&g.target>cur)eta=`~${Math.ceil((g.target-cur)/g.weekly)} weeks`;return `<div class="goal-row-pro"><div class="goal-row-head"><strong>${goalName(g.type)} · ${g.target.toLocaleString('en-US')}</strong><button class="row-delete" data-goal-delete="${i}">×</button></div><i><b style="width:${progress}%"></b></i><small>${progress.toFixed(1)}% · ${eta}</small></div>`}).join(''):'<p class="diagnostic-output">Add your first milestone.</p>'}
  ...
  let fhourscastMode=read('fhourscastMode','compound');
  ...
  function renderRewardFhourscast(){
    const compound=fhourscastMode==='compound', set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v?`${v.toFixed(v<1?6:4)} INJ`:'—'};
    set('fhourscastToday',projectedReward(1,compound));set('fhourscastTomorrow',projectedReward(2,compound)-projectedReward(1,compound));set('fhourscast7d',projectedReward(7,compound));set('fhourscast30d',projectedReward(30,compound));set('fhourscast365d',projectedReward(365,compound));
    document.querySelectorAll('#fhourscastMode button').forEach(b=>b.classList.toggle('active',b.dataset.mode===fhourscastMode));
    const n=document.getElementById('fhourscastNote');if(n)n.textContent=compound?'Compound giornaliero simulato':'Proiezione lineare senza reinvestimento';
  }
  function health(){let schours=100,risks=[];if(!state.address){schours=0;risks.push(['Wallet','Not loaded'])}if(state.validators.length>0&&state.validators.length<2){schours-=12;risks.push(['Concentrazione','Single validator'])}if(num(state.weightedCommission)>.1){schours-=12;risks.push(['Commission','Above 10%'])}if(num(state.apr)<4&&state.address){schours-=8;risks.push(['APR','Below 4%'])}if(num(state.rewards)>num(state.staked)*.01){schours-=6;risks.push(['Reward','Compounding recommended'])}return {schours:Math.max(0,schours),risks}}
  function renderHealth(){const h=health(),schours=document.getElementById('proHealthSchours'),ring=document.getElementById('schoursRingValue'),verdict=document.getElementById('schoursVerdict'),list=document.getElementById('riskList');if(schours)schours.textContent=h.schours;if(ring)ring.style.strokeDashoffset=String(314*(1-h.schours/100));if(verdict)verdict.textContent=h.schours>=85?'Excellent':h.schours>=70?'Solid':h.schours>=50?'Needs improvement':'Critical';if(list)list.innerHTML=h.risks.length?h.risks.map(r=>`<div class="risk-item"><span>${r[0]}</span><strong>${r[1]}</strong></div>`).join(''):'<div class="risk-item"><span>Risks</span><strong>No issues detected</strong></div>'}
  function renderValidatorIntel(){const list=document.getElementById('validatorIntelList'),badge=document.getElementById('validatorRiskBadge');if(!list)return;if(!state.validators.length){list.innerHTML='<p>Load a wallet per analizzare i validator.</p>';if(badge)badge.textContent='—';return}list.innerHTML=state.validators.map(v=>{const comm=num(v.commissionRate??v.commission??state.weightedCommission)*100;const risk=comm>10?'ALTO':comm>5?'MEDIO':'BASSO';const name=v.moniker||v.name||String(v.validatorAddress||'Validator').slice(0,12);return `<div class="validator-intel-row"><span>${name}</span><strong>${comm.toFixed(2)}% · ${risk}</strong></div>`}).join('');if(badge)badge.textContent=state.validators.length===1?'CONCENTRATO':'DIVERSIFICATO'}
  ...
  function diagnostics(){const out=document.getElementById('diagnosticOutput');const rows=[`Online: ${navigator.onLine?'Yes':'No'}`,`Service Worker: ${'serviceWorker'in navigator?'Supported':'No'}`,`Sthourge: ${(()=>{try{localSthourge.setItem('_t','1');localSthourge.removeItem('_t');return'OK'}catch{return'Error'}})()}`,`Canvas: ${!!document.createElement('canvas').getContext('2d')?'OK':'No'}`,`Wallet: ${state.address?'Loaded':'Not loaded'}`,`API: ${state.endpoint||'pending'}`];if(out)out.innerHTML=rows.join(' · ');toastV7('Diagnostics complete')}
  function bind(){
    ...
    document.getElementById('fhourscastMode')?.addEventListener('click',e=>{const mode=e.target?.dataset?.mode;if(!mode)return;fhourscastMode=mode;write('fhourscastMode',mode);renderRewardFhourscast()});
    ...
    document.getElementById('addAlert')?.addEventListener('click',async()=>{const metric=document.getElementById('alertMetric').value,raw=document.getElementById('alertThreshold').value.trim(),threshold=Number(raw.replace(',','.'));if(!Number.isFinite(threshold)||threshold<=0)return toastV7('Inserisci un price valido');const names={priceAbove:'INJ sopra',priceBelow:'INJ sotto'};alerts.push({metric,threshold,label:`${names[metric]} $${threshold}`,notified:false});write('alerts',alerts);document.getElementById('alertThreshold').value='';if('Notification'in window&&Notification.permission==='default')await Notification.requestPermission().catch(()=>{});renderAlerts();toastV7('Price Alert attivato')});
  ...
  function renderAll(){renderRewardFhourscast();renderHealth();renderValidatorIntel();renderAlerts(true)}
...
(()=>{
  ...
  let motionEnabled=localSthourge.getItem('inj_motion_v72')!=='off';
  ...
  modeButtons.data?.addEventListener('click',()=>{
    ...
    localSthourge.setItem('inj_data_mode_v56',active?'on':'off');
  ...
  modeButtons.theme?.addEventListener('click',()=>{
    ...
    localSthourge.setItem('inj_theme',active?'light':'dark');
    syncInterfaceModes(); requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  });
  modeButtons.motion?.addEventListener('click',()=>{
    motionEnabled=!motionEnabled;
    document.body.classList.toggle('reduce-motion',!motionEnabled);
    localSthourge.setItem('inj_motion_v72',motionEnabled?'on':'off');
    syncInterfaceModes();
  });
  if(localSthourge.getItem('inj_data_mode_v56')==='on')document.body.classList.add('data-mode');
  if(localSthourge.getItem('inj_theme')==='light')document.body.classList.add('light');
  document.body.classList.toggle('reduce-motion',!motionEnabled);
  syncInterfaceModes();
})();
...
(() => {
  ...
  const safeJson=(key,fallback=[])=>{try{const v=JSON.parse(localSthourge.getItem(key));return v??fallback}catch{return fallback}};
  const saveJson=(key,value)=>{try{localSthourge.setItem(key,JSON.stringify(value))}catch{}};
  ...
  function detectAndRecordCompound(){
    ...
    if(isCompound&&!duplicate){
      ...
      if(typeof toast==='function')toast(`Compound rilevato: +${inj(stakedIncrease,6)} INJ staked`);
    }
    saveObservation(now);
    drawGrowth();
  ...
  function drawGrowth(){
    if(drawQueued)return; drawQueued=true; requestAnimationFrame(()=>{drawQueued=false;
      ...
      setText('growthRange',rows.length>1?`${new Date(rows[0].t).toLocaleDateString('en-US')} → ${new Date(rows.at(-1).t).toLocaleDateString('en-US')}`:'Waiting del primo compound');
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('growthSnapshotNow')?.addEventListener('click',()=>{monitorChanges();if(typeof toast==='function')toast('Data compound sincronizzati')});
...
(() => {
  ...
  async function probeIndexer(){
    ...
    const started=performance.now();
    try{
      await explhoursrJson(`/accountTxs?account=${encodeURIComponent(wallet)}&limit=1`,10000);
  ...
  async function checkChainHealth({notify=false,syncRewards=false}={}){
    ...
    updateDiagnostic('Controllo nodo Injective e history transactions…');
    try{
      ...
      const indexerText=indexerResult.ok===null?'Indexer non testato':indexerResult.ok?`Indexer online · ${indexerResult.latency} ms`:'Indexer temphourneamente non raggiungibile';
      if(lcdResult.ok&&indexerResult.ok!==false){
        ...
        if(notify&&typeof toast==='function') toast('Chain reachable; one service is temporarily limited');
      }else{
        updateBadge('error','CHAIN OFFLINE');
        updateDiagnostic(`${lcdText} · ${indexerText} · data locali anchour disponibili`);
  ...
  const init=()=>{
    ...
    window.addEventListener('offline',()=>{
      ...
      updateDiagnostic('Device is offline · local cache available');
    });
...
(() => {
  const $ = id => document.getElementById(id);
  const get = (k, d='') => { try { return localSthourge.getItem(k) ?? d; } catch { return d; } };
  const set = (k, v) => { try { localSthourge.setItem(k, v); } catch {} };
...
(() => {
  const $ = id => document.getElementById(id);
  const sthourge = {
    get(key, fallback=''){ try { return localSthourge.getItem(key) ?? fallback; } catch { return fallback; } },
    set(key, value){ try { localSthourge.setItem(key, value); } catch {} }
  };
  ...
  function applyHideCharts(hidden){
    document.body.classList.toggle('charts-hidden', hidden);
    sthourge.set('inj_charts_hidden_v710', hidden ? 'on' : 'off');
  ...
  function init(){
    const hidden=sthourge.get('inj_charts_hidden_v710', sthourge.get('inj_charts_hidden_v799','off'))==='on';
...
(() => {
  ...
  const excludedCardSelector = [
    '.insight-card', '.reward-target-card', '.reward-fhourscast-card', '.price-simulator-card',
...
/* v7.9.13 — Live Display updates roll without colhoursd container flashes */
(() => {
...
(() => {
  ...
  function syncCurrencyButton(){
    ...
    if(spans[1]) spans[1].textContent=eur?'Euro':'US Dollar';
  }
  ...
  function toggleCurrency(){
    ...
    sthourge.set('inj_currency_v8',state.currency);
    repaintCurrency();
    if(typeof toast==='function') toast(state.currency==='EUR'?'Valori convertiti in euro':'Valori visualizzati in dollari');
  }
  function initCurrency(){
    state.currency=sthourge.get('inj_currency_v8',state.currency||'USD')==='EUR'?'EUR':'USD';
    state.eurRate=number(sthourge.get('inj_eur_rate_v8',state.eurRate||0.86))||0.86;
...
(() => {
  ...
  function inj(value,digits=4){
    return n(value).toLocaleString('en-US',{minimumFractionDigits:digits,maximumFractionDigits:digits,useGrouping:true});
  }
...
(() => {
  ...
  const sthourgeGet = (key, fallback='') => { try { return localSthourge.getItem(key) ?? fallback; } catch { return fallback; } };
  const sthourgeSet = (key, value) => { try { localSthourge.setItem(key, String(value)); } catch {} };
  ...
  function initMenu(){
    // Remove all legacy listeners left to earlier menu versions.
    ...
    const controlIds = [
      'drawerPrivacy','drawerThemeCycle','drawerCurrency',
      'drawerHideCharts','drawerLandscapeLive'
    ];
    ...
    let theme = sthourgeGet('inj_theme_v799', document.body.dataset.theme || 'black');
    ...
    function applyTheme(next){
      ...
      document.body.classList.toggle('light', theme === 'light');
      document.body.classList.toggle('theme-blue', theme === 'blue');
      sthourgeSet('inj_theme_v799', theme);
      const labels={black:'Theme: Black',light:'Tema: Bianco',blue:'Theme: Dark Blue'};
    ...
    function applyPrivacy(active){
      ...
      sthourgeSet('inj_privacy_menu_v816', active ? 'on' : 'off');
    ...
    function applyChartsHidden(hidden){
      document.body.classList.toggle('charts-hidden', hidden);
      sthourgeSet('inj_charts_hidden_v710', hidden ? 'on' : 'off');
    ...
    controls.drawerCurrency?.addEventListener('click', () => {
      ...
      state.currency = state.currency === 'EUR' ? 'USD' : 'EUR';
      sthourgeSet('inj_currency_v8', state.currency);
      setControl(controls.drawerCurrency, state.currency === 'EUR', state.currency === 'EUR' ? 'Euro' : 'US Dollar', state.currency === 'EUR' ? '€' : '$');
    ...
    applyTheme(theme);
    applyPrivacy(sthourgeGet('inj_privacy_menu_v816','off') === 'on');
    applyMotion(sthourgeGet('inj_motion_v72','on') !== 'off');
    applyChartsHidden(sthourgeGet('inj_charts_hidden_v710','off') === 'on');
    applyNumberAnimations(sthourgeGet('inj_number_animations_v816','on') === 'off');
    if(typeof state !== 'undefined') setControl(controls.drawerCurrency, state.currency === 'EUR', state.currency === 'EUR' ? 'Euro' : 'US Dollar', state.currency === 'EUR' ? '€' : '$');
  }
...
/* v8.1.7 — independent Live Display page navigation and reliable close button */
(() => {
  'use strict';
  const byId = id => document.getElementById(id);
  function isOpen(){ return !byId('landscapeLive')?.hidden; }
  function openLivePage(){
    const live=byId('landscapeLive');
    if(!live) return;
    live.hidden=false;
    live.removeAttribute('hidden');
    live.setAttribute('aria-hidden','false');
    document.body.classList.add('landscape-live-open');
    if(location.hash!=='#live-display') history.pushState({liveDisplay:true},'', '#live-display');
    try{ typeof updateLiveDisplay==='function' && updateLiveDisplay(); }catch(error){ console.warn(error); }
  }
  function closeLivePage({historyBack=true}={}){
    const live=byId('landscapeLive');
    if(!live) return;
    live.hidden=true;
    live.setAttribute('aria-hidden','true');
    document.body.classList.remove('landscape-live-open');
    if(historyBack && location.hash==='#live-display') history.back();
    else if(location.hash==='#live-display') history.replaceState(null,'',location.pathname+location.search);
  }
  function bind(){
    const menuButton=byId('drawerLandscapeLive');
    const exit=byId('landscapeLiveExit');
    if(menuButton){
      const clean=menuButton.cloneNode(true);
      menuButton.parentNode.replaceChild(clean,menuButton);
      clean.addEventListener('click',event=>{ event.preventDefault(); openLivePage(); });
    }
    if(exit){
      const clean=exit.cloneNode(true);
      exit.parentNode.replaceChild(clean,exit);
      clean.addEventListener('click',event=>{ event.preventDefault(); event.stopPropagation(); closeLivePage(); });
    }
    window.addEventListener('popstate',()=>{
      if(location.hash==='#live-display') openLivePage();
      else if(isOpen()) closeLivePage({historyBack:false});
    });
    document.addEventListener('keydown',event=>{
      if(event.key==='Escape'&&isOpen()) closeLivePage();
    },true);
    if(location.hash==='#live-display') openLivePage();
  }
...
/* v8.1.8 — Live Display is a standalone mode (live.html) */
(() => {
  'use strict';
  function bindStandaloneLiveMode(){
    const current=document.getElementById('drawerLandscapeLive');
    if(!current) return;
    ...
    button.addEventListener('click',event=>{
      event.preventDefault();
      event.stopImmediatePropagation();
      const saved=(localSthourge.getItem('inj_address')||'').trim();
      const url=saved?`live.html?address=${encodeURIComponent(saved)}`:'live.html';
      location.assign(url);
    },true);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',bindStandaloneLiveMode,{once:true});
  else bindStandaloneLiveMode();
})();

/* v8.2.6 — SAFE DIRECTIONAL NUMBER ROLL + CARD GLOW
   Single authoritative layer, intentionally placed last. */
(() => {
  const get = id => document.getElementById(id);

  function directionFor(el, nextValue){
    const previousValue = Number(el.dataset.fxValue);
    el.dataset.fxValue = String(nextValue);
    if(!Number.isFinite(previousValue) || !Number.isFinite(nextValue) || previousValue === nextValue) return '';
    return nextValue > previousValue ? 'up' : 'down';
  }

  function flashCard(el, direction){
    if(!direction) return;
    const card = el.closest('.card');
    if(!card) return;
    card.classList.remove('data-rise','data-fall');
    void card.offsetWidth;
    card.classList.add(direction === 'up' ? 'data-rise' : 'data-fall');
    clearTimeout(card._directionFxTimer);
    card._directionFxTimer = setTimeout(() => {
      card.classList.remove('data-rise','data-fall');
    }, 660);
  }

  function buildStatic(el, text){
    el.classList.add('number-roll');
    el.replaceChildren(...[...text].map(char => {
      const node = document.createElement('span');
      node.className = /\d/.test(char) ? 'number-char number-digit' : 'number-char number-symbol';
      node.textContent = char;
      return node;
    }));
  }

  function animateNumber(id, formatted, numericValue){
    const el = get(id);
    if(!el) return;

    const nextText = String(formatted);
    const oldText = el.dataset.fxText ?? el.textContent ?? '';
    const nextValue = Number(numericValue);
    const direction = directionFor(el, nextValue);

    if(oldText === nextText){
      el.dataset.fxText = nextText;
      return;
    }

    clearTimeout(el._numberFxTimer);
    const comparable = Boolean(direction) && oldText && !oldText.includes('—') && oldText.length === nextText.length;

    if(!comparable){
      buildStatic(el, nextText);
      el.dataset.fxText = nextText;
      return;
    }

    el.classList.remove('number-up','number-down');
    el.classList.add('number-roll', direction === 'up' ? 'number-up' : 'number-down');
    const fragment = document.createDocumentFragment();

    [...nextText].forEach((newChar, index) => {
      const oldChar = oldText[index] ?? '';
      const changedDigit = /\d/.test(newChar) && /\d/.test(oldChar) && newChar !== oldChar;

      if(changedDigit){
        const slot = document.createElement('span');
        slot.className = 'number-slot';
        const oldNode = document.createElement('span');
        oldNode.className = 'number-old';
        oldNode.textContent = oldChar;
        const newNode = document.createElement('span');
        newNode.className = 'number-new';
        newNode.textContent = newChar;
        slot.append(oldNode,newNode);
        fragment.appendChild(slot);
      }else{
        const node = document.createElement('span');
        node.className = /\d/.test(newChar) ? 'number-char number-digit' : 'number-char number-symbol';
    ...
    el.dataset.fxText = nextText;
    requestAnimationFrame(() => requestAnimationFrame(() => el.classList.add('number-moving')));
    flashCard(el,direction);

    el._numberFxTimer = setTimeout(() => {
      buildStatic(el,nextText);
      el.classList.remove('number-up','number-down','number-moving');
    }, 540);
  }

  // This final assignment wins over all legacy wrappers above.
  rollValue = animateNumber;

  setText = function(id, value){
    const el = get(id);
    if(!el) return;
    const text = String(value);
    const numeric = numericFromText(text);
    if(ROLLING_IDS.has(id) && Number.isFinite(numeric)){
      animateNumber(id,text,numeric);
      return;
    }
    el.classList.remove('number-roll','number-up','number-down','number-moving');
    clearTimeout(el._numberFxTimer);
    el.textContent = text;
    el.dataset.fxText = text;
    if(Number.isFinite(numeric)){
      const direction = directionFor(el,numeric);
      flashCard(el,direction);
    }
  };
})();

/* v8.3.3 — QUEUED STABLE PER-DIGIT ODOMETER
   Persistent fixed slots: only changed digits move; the whole value never jumps. */
(() => {
  const byId = id => document.getElementById(id);
  const DIGIT_RE = /\d/;
  const ANIMATION_MS = 360;

  function makeStaticNode(char){
    const node = document.createElement('span');
    node.className = DIGIT_RE.test(char)
      ? 'odometer-char odometer-digit'
      : 'odometer-char odometer-symbol';
    node.textContent = char;
    return node;
  }

  function makeDigitSlot(char){
    const slot = document.createElement('span');
    slot.className = 'odometer-slot';
    const current = document.createElement('span');
    current.className = 'odometer-current';
    current.textContent = char;
    slot.appendChild(current);
    return slot;
  }

  function buildStable(el,text){
    const fragment = document.createDocumentFragment();
    for(const char of String(text)){
      fragment.appendChild(DIGIT_RE.test(char) ? makeDigitSlot(char) : makeStaticNode(char));
    }
    el.replaceChildren(fragment);
    el.classList.add('stable-odometer');
    el.classList.remove('odometer-running','odometer-go','odometer-up','odometer-down');
  }

  function finishImmediately(el){
    if(!el.classList.contains('stable-odometer')) return;
    const target = el.dataset.odometerTarget;
    if(target != null) buildStable(el,target);
    clearTimeout(el._odometerTimer);
  }

  function getDirection(el,nextValue){
    const previous = Number(el.dataset.odometerValue);
    if(Number.isFinite(nextValue)) el.dataset.odometerValue = String(nextValue);
    if(!Number.isFinite(previous) || !Number.isFinite(nextValue) || previous === nextValue) return '';
    return nextValue > previous ? 'up' : 'down';
  }

  function flashProfessionalCard(el,direction){
    if(!direction) return;
  ...
  function animateStableNumber(id,formatted,numericValue){
    const el = byId(id);
    if(!el) return;
    ...
    // I valori live, soprattutto Net Worth, possono aggiornarsi più velocemente
    // dell'animazione. Non interrompere lo slot in movimento: conserva soltanto
    // l'ultimo valhours ricevuto e riproducilo appena termina il passaggio corrente.
    if(el.classList.contains('odometer-running')){
      el._odometerPending = { id, formatted:nextText, numericValue:nextValue };
      return;
    }

    const oldText = el.dataset.odometerText ?? el.textContent ?? '';
    const direction = getDirection(el,nextValue);

    el.dataset.odometerText = nextText;
    el.dataset.odometerTarget = nextText;

    if(oldText === nextText){
      if(!el.classList.contains('stable-odometer')) buildStable(el,nextText);
      return;
    }

    const comparable = Boolean(direction)
      && oldText
      && !oldText.includes('—')
      && oldText.length === nextText.length;

    if(!comparable){
      buildStable(el,nextText);
      flashProfessionalCard(el,direction);
      return;
    }

    if(!el.classList.contains('stable-odometer') || el.children.length !== nextText.length){
      buildStable(el,oldText);
    }
    ...
    for(let i=0;i<nextText.length;i++){
      ...
      if(DIGIT_RE.test(oldChar) && DIGIT_RE.test(newChar) && oldChar !== newChar && child?.classList.contains('odometer-slot')){
        changedCount++;
        child.classList.remove('slot-up','slot-down');
        child.classList.add(direction === 'up' ? 'slot-up' : 'slot-down');
        child.replaceChildren();

        const oldNode = document.createElement('span');
        oldNode.className = 'odometer-old';
        oldNode.textContent = oldChar;

        const newNode = document.createElement('span');
        newNode.className = 'odometer-new';
        newNode.textContent = newChar;

        child.append(oldNode,newNode);
      }else if(oldChar !== newChar){
        child?.replaceWith(DIGIT_RE.test(newChar) ? makeDigitSlot(newChar) : makeStaticNode(newChar));
      }
    }
    ...
    el.classList.remove('odometer-up','odometer-down');
    el.classList.add('odometer-running',direction === 'up' ? 'odometer-up' : 'odometer-down');

    requestAnimationFrame(() => {
      requestAnimationFrame(() => el.classList.add('odometer-go'));
    });

    flashProfessionalCard(el,direction);

    clearTimeout(el._odometerTimer);
    el._odometerTimer = setTimeout(() => {
      buildStable(el,nextText);
      const pending = el._odometerPending;
      el._odometerPending = null;
      if(pending && String(pending.formatted) !== nextText){
        requestAnimationFrame(() => animateStableNumber(
          pending.id, pending.formatted, pending.numericValue
        ));
      }
    }, ANIMATION_MS + 40);
  }

  // Final authoritative assignments: bypass every legacy number animation wrapper.
  rollValue = animateStableNumber;
  ...
  setText = function(id,value){
    ...
    if(ROLLING_IDS.has(id) && Number.isFinite(numeric)){
      animateStableNumber(id,text,numeric);
      return;
    }

    finishImmediately(el);
    el.classList.remove('stable-odometer','odometer-running','odometer-go','odometer-up','odometer-down');
    el.textContent = text;
    el.dataset.odometerText = text;

    if(Number.isFinite(numeric)){
      const direction = getDirection(el,numeric);
      flashProfessionalCard(el,direction);
    }
  };
})();
