...
function money(value){ return new Intl.NumberFormat(displayCurrency==='EUR'?'it-IT':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:2,maximumFractionDigits:2}).format(currencyValue(value)); }
function preciseMoney(value,digits=5){ return new Intl.NumberFormat(displayCurrency==='EUR'?'it-IT':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:digits,maximumFractionDigits:digits}).format(currencyValue(value)); }
function compactUsd(value){ return new Intl.NumberFormat(displayCurrency==='EUR'?'it-IT':'en-US',{style:'currency',currency:displayCurrency,notation:'compact',maximumFractionDigits:2}).format(currencyValue(value)); }
...
(() => {
  ...
  function updateRewardTarget(){const input=document.getElementById('dailyRewardTargetUsd'),out=document.getElementById('requiredInjPrice');if(!input||!out)return;const target=Math.max(0,Number(input.value)||0),daily=Number(state?.staked||0)*(Number(state?.apr||0)/100)/365;const symbol=document.querySelector('.reward-target-input b');if(symbol)symbol.textContent=displayCurrency==='EUR'?'€':'$';if(!(daily>0)&&target>0){out.textContent='—';return;}const required=target>0?target/daily:0;out.textContent=new Intl.NumberFormat(displayCurrency==='EUR'?'it-IT':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:2,maximumFractionDigits:2}).format(required);setText('rewardTargetStaked',`${inj(state?.staked||0,4)} INJ`);setText('rewardTargetDailyInj',`${inj(daily,6)} INJ`);setText('rewardTargetApr',state?.apr?`${Number(state.apr).toFixed(2)}%`:'—');const summary=document.getElementById('rewardTargetSummary');if(summary)summary.textContent='';}
...
(()=>{
  ...
  function init(){
    ...
    const syncHide=()=>{document.body.classList.toggle('charts-hidden',hidden);hide?.classList.toggle('active',hidden);hide?.setAttribute('aria-pressed',String(hidden));if(hide)hide.querySelector('span:last-child').textContent=hidden?'Mostra grafici':'Nascondi grafici';requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')))};
...
(() => {
  ...
  const LABELS={black:'Tema: Nero',light:'Tema: Chiaro',blue:'Tema: Blu scuro'};
  ...
  function moneyLive(value){
    try{return typeof money==='function'?money(value):new Intl.NumberFormat('it-IT',{style:'currency',currency:'USD'}).format(value||0)}catch{return `$${Number(value||0).toFixed(2)}`}
  }
  function syncLiveDisplay(){
    ...
    set('landscapeWalletInj',`${total.toLocaleString('it-IT',{minimumFractionDigits:4,maximumFractionDigits:4})} INJ`);
    set('landscapeWalletAvailable',Number(state?.available||0).toLocaleString('it-IT',{minimumFractionDigits:4,maximumFractionDigits:4}));
    set('landscapeWalletStaked',Number(state?.staked||0).toLocaleString('it-IT',{minimumFractionDigits:4,maximumFractionDigits:4}));
    set('landscapeWalletRewards',Number(state?.rewards||0).toLocaleString('it-IT',{minimumFractionDigits:6,maximumFractionDigits:6}));
  }
  function init(){
