...
/* v7.9.8 — robust drawer controls and Daily Reward Target repair */
(() => {
  const $id=id=>document.getElementById(id);
  const store=(key,value)=>{try{localStorage.setItem(key,value)}catch{}};
  const read=(key,fallback='')=>{try{return localStorage.getItem(key)??fallback}catch{return fallback}};
  const closeDrawer=()=>{
    const drawer=$id('mainDrawer'),backdrop=$id('drawerBackdrop'),toggle=$id('menuToggle');
    drawer?.classList.remove('open'); drawer?.setAttribute('aria-hidden','true');
    toggle?.setAttribute('aria-expanded','false'); document.body.classList.remove('drawer-open');
    if(backdrop){backdrop.classList.remove('visible');setTimeout(()=>{if(!drawer?.classList.contains('open'))backdrop.hidden=true},220)}
  };
  const setButton=(id,active,label,icon)=>{
    const button=$id(id); if(!button)return;
    button.classList.toggle('active',Boolean(active));
    button.setAttribute('aria-pressed',active?'true':'false');
    const spans=button.querySelectorAll('span');
    if(icon&&spans[0])spans[0].textContent=icon;
    if(label&&spans[1])spans[1].textContent=label;
  };
  const themes=['black','light','blue'];
  const themeLabels={black:'Theme: Black',light:'Theme: Light',blue:'Theme: Dark Blue'};
  const applyTheme=theme=>{
    const safe=themes.includes(theme)?theme:'black';
    document.body.classList.toggle('light',safe==='light');
    document.body.classList.toggle('theme-blue',safe==='blue');
    document.body.dataset.theme=safe;
    store('inj_theme_cycle_v798',safe);
    setButton('drawerThemeCycle',safe!=='black',themeLabels[safe],safe==='light'?'☀':safe==='blue'?'●':'●');
    requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  };
  const togglePrivacy=()=>{
    const active=document.body.classList.toggle('privacy');
    store('inj_privacy_v798',active?'on':'off');
    setButton('drawerPrivacy',active,'Privacy',active?'◌':'◉');
  };
  const toggleCharts=()=>{
    const active=document.body.classList.toggle('charts-hidden');
    store('inj_charts_hidden_v798',active?'on':'off');
    setButton('drawerHideCharts',active,active?'Show charts':'Hide charts','▱');
    requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  };
  const toggleAnimations=()=>{
    const active=document.body.classList.toggle('reduce-motion');
    store('inj_motion_v72',active?'off':'on');
    setButton('drawerHideAnimations',active,active?'Animations off':'Animations on','↕');
    setButton('drawerMotionButton',!active,'Effects',!active?'✦':'·');
  };
  const toggleCurrency=()=>{
    const current=read('inj_currency_v798','USD');
    const next=current==='USD'?'EUR':'USD';
    store('inj_currency_v798',next);
    document.body.dataset.currency=next;
    setButton('drawerCurrency',next==='EUR',next==='EUR'?'Euro':'Dollar',next==='EUR'?'€':'$');
    document.dispatchEvent(new CustomEvent('inj:currency-changed',{detail:{currency:next}}));
    if(typeof drawAll==='function')try{drawAll()}catch{}
  };
  const openDialog=id=>{closeDrawer();setTimeout(()=>{const dialog=$id(id);if(dialog?.showModal&&!dialog.open)dialog.showModal()},60)};
  const scrollTo=id=>{closeDrawer();setTimeout(()=>$id(id)?.scrollIntoView({behavior:'smooth',block:'start'}),70)};

  function handleMenuClick(event){
    const button=event.target.closest('#mainDrawer button');
    if(!button)return;
    const id=button.id;
    ...
    if(id==='drawerClose'){closeDrawer();return}
    if(id==='drawerPrivacy'){togglePrivacy();return}
    if(id==='drawerThemeCycle'){
      const current=document.body.dataset.theme||read('inj_theme_cycle_v798','black');
      applyTheme(themes[(themes.indexOf(current)+1)%themes.length]);return;
    }
    if(id==='drawerCurrency'){toggleCurrency();return}
    if(id==='drawerMotionButton'||id==='drawerHideAnimations'){toggleAnimations();return}
    if(id==='drawerHideCharts'){toggleCharts();return}
    if(id==='drawerLandscapeLive'){
      closeDrawer();
      const panel=$id('landscapeLive');
      if(panel){panel.hidden=false;panel.setAttribute('aria-hidden','false');document.body.classList.add('landscape-live-open')}
      return;
    }
    if(action==='dashboard'){scrollTo('dashboardView');return}
    if(action==='manage-wallets'||action==='wallets'){openDialog('walletManagerDialog');return}
    if(action==='alerts'){openDialog('alertCenterDialog');return}
    if(action==='data'){scrollTo('proSuite');return}
  }

  function renderDailyRewardTarget(){
    const input=$id('dailyRewardTargetUsd');
    if(!input)return;
    const target=Math.max(0,Number(input.value)||0);
    const staked=Math.max(0,Number(state?.staked)||0);
    const apr=Math.max(0,Number(state?.apr)||0);
    const dailyInj=staked*(apr/100)/365;
    const required=dailyInj>0?target/dailyInj:0;
    if(typeof rollValue==='function'){
      rollValue('requiredInjPrice',dailyInj>0?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(required):'—',required);
      rollValue('rewardTargetStaked',staked>0?`${staked.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:4})} INJ`:'—',staked);
      rollValue('rewardTargetDailyInj',dailyInj>0?`${dailyInj.toLocaleString('en-US',{minimumFractionDigits:6,maximumFractionDigits:6})} INJ`:'—',dailyInj);
    }else{
      $id('requiredInjPrice').textContent=dailyInj>0?`$${required.toFixed(2)}`:'—';
      $id('rewardTargetStaked').textContent=staked>0?`${staked.toFixed(4)} INJ`:'—';
      $id('rewardTargetDailyInj').textContent=dailyInj>0?`${dailyInj.toFixed(6)} INJ`:'—';
    }
    const aprEl=$id('rewardTargetApr');
    if(aprEl){aprEl.textContent=apr>0?`${apr.toFixed(2)}%`:'—';aprEl.classList.add('fixed-percentage')}
  }

  function init(){
    document.addEventListener('click',handleMenuClick,true);
    const input=$id('dailyRewardTargetUsd');
    input?.addEventListener('input',renderDailyRewardTarget);
    input?.addEventListener('change',renderDailyRewardTarget);
    applyTheme(read('inj_theme_cycle_v798',document.body.classList.contains('light')?'light':'black'));
    const privacy=read('inj_privacy_v798','off')==='on';document.body.classList.toggle('privacy',privacy);setButton('drawerPrivacy',privacy,'Privacy',privacy?'◌':'◉');
    const hidden=read('inj_charts_hidden_v798','off')==='on';document.body.classList.toggle('charts-hidden',hidden);setButton('drawerHideCharts',hidden,hidden?'Show charts':'Hide charts','▱');
    const reduced=read('inj_motion_v72','on')==='off';document.body.classList.toggle('reduce-motion',reduced);setButton('drawerHideAnimations',reduced,reduced?'Animations off':'Animations on','↕');setButton('drawerMotionButton',!reduced,'Effects',!reduced?'✦':'·');
    const currency=read('inj_currency_v798','USD');document.body.dataset.currency=currency;setButton('drawerCurrency',currency==='EUR',currency==='EUR'?'Euro':'Dollar',currency==='EUR'?'€':'$');
    renderDailyRewardTarget();
    setInterval(renderDailyRewardTarget,1500);
  }
