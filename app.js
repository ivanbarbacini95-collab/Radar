...
let displayCurrency=localStorage.getItem('inj_display_currency_v789')==='EUR'?'EUR':'USD';
let eurUsdRate=Number(localStorage.getItem('inj_eur_usd_rate_v789'))||0.92;
function currencyValue(value){ return displayCurrency==='EUR'?number(value)*eurUsdRate:number(value); }
function money(value){ return new Intl.NumberFormat(displayCurrency==='EUR'?'en-US':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:2,maximumFractionDigits:2}).format(currencyValue(value)); }
function preciseMoney(value,digits=5){ return new Intl.NumberFormat(displayCurrency==='EUR'?'en-US':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:digits,maximumFractionDigits:digits}).format(currencyValue(value)); }
function compactUsd(value){ return new Intl.NumberFormat(displayCurrency==='EUR'?'en-US':'en-US',{style:'currency',currency:displayCurrency,notation:'compact',maximumFractionDigits:2}).format(currencyValue(value)); }
...
function rollValue(id,formatted,numericValue){
  const el=$(id); if(!el) return;
  if(document.body.classList.contains('privacy')){ el.textContent=String(formatted); el.dataset.rollText=String(formatted); if(Number.isFinite(Number(numericValue)))el.dataset.rollValue=String(numericValue); return; }
...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    storage.set('inj_address',address); setText('lastUpdate',`Aggiornato ${new Date().toLocaleTimeString('en-US')}`);
...
const WALLET_CYCLE_MS=24*60*60*1000;
let walletCycleUnit=localStorage.getItem('inj_wallet_cycle_unit_v789')||'percentuale';
function walletCycleKey(address){return `inj_wallet_cycle_24h_v789_${String(address||'preview').toLowerCase()}`}
function loadWalletCycle(){
  const fallback={startedAt:0,baselineUsd:0,history:[]};
  const data=storage.getJSON(walletCycleKey(state.address),fallback)||fallback;
  data.history=Array.isArray(data.history)?data.history:[];
  return {...fallback,...data};
}
function drawWalletCycle(cycle,currentUsd){
  const canvas=$('walletCycleChart'); if(!canvas)return;
  const rect=canvas.getBoundingClientRect(), dpr=Math.max(1,window.devicePixelRatio||1), w=Math.max(620,Math.max(rect.width,cycle.history.length*52+100)), h=Math.max(250,rect.height||280);
  ...
  const pct=cycle.baselineUsd?((currentUsd-cycle.baselineUsd)/cycle.baselineUsd)*100:0;
  const usd=currentUsd-cycle.baselineUsd;
  const rows=[...cycle.history.slice(-30),{t:Date.now(),pct,usd,current:true}];
  const values=rows.map(r=>walletCycleUnit==='valuta'?r.usd:r.pct);
  const maxAbs=Math.max(1e-6,...values.map(Math.abs));const zero=h/2,padX=40,barW=Math.max(10,Math.min(28,(w-padX*2)/Math.max(1,rows.length)*.58));
  ctx.strokeStyle='rgba(148,163,184,.55)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(18,zero);ctx.lineTo(w-18,zero);ctx.stroke();
  ctx.fillStyle='rgba(148,163,184,.9)';ctx.font='11px system-ui';ctx.fillText('0',20,zero-7);
  rows.forEach((r,i)=>{const v=values[i];const x=padX+i*((w-padX*2)/Math.max(1,rows.length));const bh=Math.max(v===0?2:4,Math.abs(v)/maxAbs*(h*.39));const y=v>=0?zero-bh:zero;ctx.fillStyle=v>=0?'#22d3a6':'#fb7185';ctx.globalAlpha=r.current?1:.72;ctx.fillRect(x-barW/2,y,barW,bh);ctx.globalAlpha=1;if(r.current){ctx.strokeStyle=v>=0?'#6ee7c8':'#fda4af';ctx.lineWidth=2;ctx.strokeRect(x-barW/2-1,y-1,barW+2,bh+2)};if(!r.current){ctx.fillStyle='rgba(148,163,184,.8)';ctx.font='10px system-ui';ctx.textAlign='center';ctx.fillText(new Date(r.t).toLocaleDateString('en-US',{day:'2-digit',month:'2-digit'}),x,h-10)}});
  const scroller=$('walletCycleScroll');if(scroller)requestAnimationFrame(()=>scroller.scrollLeft=scroller.scrollWidth);
}
function renderWalletCycle(total,nw){
  if(!state.address||!nw){setText('walletCycleStatus','IN ATTESA');setText('walletCycleCountdown','24h 00m');setText('walletCycleLastCheck','Carica un wallet');return;}
  const now=Date.now();const cycle=loadWalletCycle();
  if(!cycle.startedAt||!cycle.baselineUsd){cycle.startedAt=now;cycle.baselineUsd=nw;storage.setJSON(walletCycleKey(state.address),cycle)}
  while(now-cycle.startedAt>=WALLET_CYCLE_MS){const endValue=nw;const usd=endValue-cycle.baselineUsd;const pct=cycle.baselineUsd?(usd/cycle.baselineUsd)*100:0;cycle.history.push({t:cycle.startedAt+WALLET_CYCLE_MS,usd,pct});cycle.history=cycle.history.slice(-90);cycle.startedAt+=WALLET_CYCLE_MS;cycle.baselineUsd=endValue;storage.setJSON(walletCycleKey(state.address),cycle)}
  const remaining=Math.max(0,WALLET_CYCLE_MS-(now-cycle.startedAt));const hours=Math.floor(remaining/3600000),mins=Math.floor((remaining%3600000)/60000);
  const usd=nw-cycle.baselineUsd,pct=cycle.baselineUsd?(usd/cycle.baselineUsd)*100:0;
  setText('walletCycleCountdown',`${hours}h ${String(mins).padStart(2,'0')}m`);setText('walletCycleLastCheck',`Candela corrente: ${walletCycleUnit==='valuta'?money(usd):`${pct>=0?'+':''}${pct.toFixed(2)}%`}`);setText('walletCycleStatus',usd>=0?'GUADAGNO':'PERDITA');
  drawWalletCycle(cycle,nw);
}
function render(){
...
function renderPriceSimulator(){
  ...
  if(delta){
    if(!price) delta.textContent='Inserisci valori validi';
      ...
      delta.textContent=`${change>=0?'+':''}${change.toLocaleString('en-US',{maximumFractionDigits:1})}% rispetto al prezzo attuale`;
...
function init(){
  ...
  state.walletRefreshTimer=setInterval(refreshInactiveWallets,60000); setTimeout(refreshInactiveWallets,5000); startMultiwalletRewardMonitor();
}
...
(() => {
  ...
  function updateTimeline(){
    ...
    if(!V5.viewing) q('timelineDate').textContent=V5.snapshots.length?`Ultimo salvataggio: ${new Date(V5.snapshots.at(-1).t).toLocaleString('en-US')}`:'Lo storico inizierà dal primo salvataggio';
  }
...
(() => {
  ...
  function pulseValue(el,oldText,newText){
    ...
    void el.offsetWidth;
    el.classList.add(oldValue!==null&&newValue!==null&&newValue<oldValue?'value-tick-down':'value-tick-up');
...
(()=>{
  ...
  const usd=v=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:2}).format(num(v));
  ...
  function renderRewardForecast(){
    ...
    const n=document.getElementById('forecastNote');if(n)n.textContent=compound?'Simulated daily compounding':'Linear projection without reinvestment';
  }
...
(() => {
  ...
  function drawGrowth(){
    if(drawQueued)return; drawQueued=true; requestAnimationFrame(()=>{drawQueued=false;
      ...
      setText('growthRange',rows.length>1?`${new Date(rows[0].t).toLocaleDateString('en-US')} → ${new Date(rows.at(-1).t).toLocaleDateString('en-US')}`:'In attesa del primo compound');
...
/* v7.8.9 — privacy, currency, daily candle and strict timeframe corrections */
(() => {
  function refreshAll(){ try{render();renderMarket();}catch(e){console.warn('Aggiornamento vista',e)} }
  async function loadEurRate(){
    try{const r=await fetch('https://api.frankfurter.app/latest?from=USD&to=EUR',{cache:'no-store'});const j=await r.json();if(Number(j?.rates?.EUR)){eurUsdRate=Number(j.rates.EUR);localStorage.setItem('inj_eur_usd_rate_v789',String(eurUsdRate));refreshAll();}}
    catch(e){console.warn('Cambio EUR non disponibile',e)}
  }
  function syncCurrencyButton(){const b=document.getElementById('drawerCurrency');if(!b)return;b.querySelector('.interface-icon').textContent=displayCurrency==='EUR'?'€':'$';b.querySelector('span:last-child').textContent=displayCurrency==='EUR'?'Euro':'Dollaro';b.classList.toggle('active',displayCurrency==='EUR');b.setAttribute('aria-pressed',displayCurrency==='EUR'?'true':'false');}
  function updateRewardTarget(){const input=document.getElementById('dailyRewardTargetUsd'),out=document.getElementById('requiredInjPrice');if(!input||!out)return;const target=Math.max(0,Number(input.value)||0),daily=Number(state?.staked||0)*(Number(state?.apr||0)/100)/365;const symbol=document.querySelector('.reward-target-input b');if(symbol)symbol.textContent=displayCurrency==='EUR'?'€':'$';if(!(daily>0)&&target>0){out.textContent='—';return;}const required=target>0?target/daily:0;out.textContent=new Intl.NumberFormat(displayCurrency==='EUR'?'en-US':'en-US',{style:'currency',currency:displayCurrency,minimumFractionDigits:2,maximumFractionDigits:2}).format(required);setText('rewardTargetStaked',`${inj(state?.staked||0,4)} INJ`);setText('rewardTargetDailyInj',`${inj(daily,6)} INJ`);setText('rewardTargetApr',state?.apr?`${Number(state.apr).toFixed(2)}%`:'—');const summary=document.getElementById('rewardTargetSummary');if(summary)summary.textContent='';}
  function strictPerformance(ms){const rows=(state.netWorthHistory||[]).filter(r=>Number(r?.t)>0&&Number(r?.v)>0).sort((a,b)=>a.t-b.t);if(rows.length<2)return null;const last=rows.at(-1),target=last.t-ms;if(rows[0].t>target)return null;let best=null;for(const r of rows){if(r.t<=target)best=r;else break;}if(!best||last.t-best.t<ms*.85)return null;return {usd:last.v-best.v,pct:(last.v/best.v-1)*100};}
  function updateStrictPerformance(){[['perf1d',86400000],['perf7d',7*86400000],['perf30d',30*86400000]].forEach(([id,ms])=>setPerformance(id,strictPerformance(ms)));const rows=(state.netWorthHistory||[]).filter(r=>Number(r?.v)>0).sort((a,b)=>a.t-b.t);setPerformance('perfAll',rows.length>1?{usd:rows.at(-1).v-rows[0].v,pct:(rows.at(-1).v/rows[0].v-1)*100}:null);}
  document.addEventListener('DOMContentLoaded',()=>{
    document.getElementById('drawerCurrency')?.addEventListener('click',()=>{displayCurrency=displayCurrency==='USD'?'EUR':'USD';localStorage.setItem('inj_display_currency_v789',displayCurrency);syncCurrencyButton();refreshAll();});
    document.querySelectorAll('[data-cycle-unit]').forEach(b=>b.addEventListener('click',()=>{walletCycleUnit=b.dataset.cycleUnit;localStorage.setItem('inj_wallet_cycle_unit_v789',walletCycleUnit);document.querySelectorAll('[data-cycle-unit]').forEach(x=>x.classList.toggle('active',x===b));refreshAll();}));
    document.querySelectorAll('[data-cycle-unit]').forEach(b=>b.classList.toggle('active',b.dataset.cycleUnit===walletCycleUnit));
    syncCurrencyButton();updateRewardTarget();if(displayCurrency==='EUR')loadEurRate();
    document.getElementById('dailyRewardTargetUsd')?.addEventListener('input',updateRewardTarget);
    const privacy=document.getElementById('drawerPrivacy');privacy?.addEventListener('click',()=>{document.body.classList.toggle('privacy-lock',document.body.classList.contains('privacy'));});
    document.body.classList.toggle('privacy-lock',document.body.classList.contains('privacy'));
    setInterval(()=>{updateStrictPerformance();updateRewardTarget();if(document.body.classList.contains('privacy'))document.body.classList.add('privacy-lock');},2500);
    setTimeout(updateStrictPerformance,1000);
  });
})();

/* v7.9.0 — restored themes, chart toggle, live display and true vertical digits */
function rollValue(id,formatted,numericValue){
  const el=document.getElementById(id); if(!el)return;
  const next=String(formatted);
  if(document.body.classList.contains('privacy')){
    el.dataset.rollText=next;
    if(Number.isFinite(Number(numericValue)))el.dataset.rollValue=String(numericValue);
    return;
  }
  const prev=el.dataset.rollText ?? el.textContent ?? '';
  const prevValue=Number(el.dataset.rollValue), nextValue=Number(numericValue);
  if(prev===next)return;
  const comparable=prev.length===next.length && Number.isFinite(prevValue)&&Number.isFinite(nextValue)&&!prev.includes('—');
  const direction=nextValue<prevValue?'down':'up';
  el.classList.add('rolling-number'); el.innerHTML='';
  for(let i=0;i<next.length;i++){
    const oldChar=prev[i]??'', newChar=next[i];
    const isDigit=/\d/.test(newChar), changed=comparable&&isDigit&&/\d/.test(oldChar)&&oldChar!==newChar;
    const slot=document.createElement('span');
    slot.className=isDigit?'roll-char roll-digit':'roll-char roll-symbol';
    if(changed){
      slot.classList.add(`digit-change-${direction}`);
      const oldNode=document.createElement('span'); oldNode.className='digit-old'; oldNode.textContent=oldChar;
      const newNode=document.createElement('span'); newNode.className='digit-new'; newNode.textContent=newChar;
      slot.append(oldNode,newNode);
    }else slot.textContent=newChar;
    el.appendChild(slot);
  }
  el.dataset.rollText=next;
  if(Number.isFinite(nextValue))el.dataset.rollValue=String(nextValue);
  clearTimeout(el._rollTimer);
  el._rollTimer=setTimeout(()=>{
    [...el.querySelectorAll('.roll-digit')].forEach(slot=>{
      const n=slot.querySelector('.digit-new'); if(n)slot.textContent=n.textContent;
      slot.classList.remove('digit-change-up','digit-change-down');
    });
  },380);
}

(()=>{
  const THEME_KEY='inj_theme_v790';
  function applyTheme(theme){
    const allowed=['black','light','blue']; if(!allowed.includes(theme))theme='black';
    document.body.classList.remove('light','theme-black','theme-light','theme-blue');
    document.body.classList.add(`theme-${theme}`);
    localStorage.setItem(THEME_KEY,theme); localStorage.setItem('inj_theme',theme==='light'?'light':'dark');
    document.querySelectorAll('[data-theme-choice]').forEach(b=>{const active=b.dataset.themeChoice===theme;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));});
    const meta=document.querySelector('meta[name="theme-color"]'); if(meta)meta.content=theme==='light'?'#f3f5f8':theme==='blue'?'#030a16':'#000000';
    requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  }
  function init(){
    const saved=localStorage.getItem(THEME_KEY)|| (localStorage.getItem('inj_theme')==='light'?'light':'black'); applyTheme(saved);
    document.querySelectorAll('[data-theme-choice]').forEach(b=>b.addEventListener('click',()=>applyTheme(b.dataset.themeChoice)));

    const hide=document.getElementById('drawerHideCharts');
    let hidden=localStorage.getItem('inj_hide_charts_v790')==='on';
    const syncHide=()=>{document.body.classList.toggle('charts-hidden',hidden);hide?.classList.toggle('active',hidden);hide?.setAttribute('aria-pressed',String(hidden));if(hide)hide.querySelector('span:last-child').textContent=hidden?'Show charts':'Hide charts';requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')))};
    hide?.addEventListener('click',()=>{hidden=!hidden;localStorage.setItem('inj_hide_charts_v790',hidden?'on':'off');syncHide()}); syncHide();

    const anim=document.getElementById('drawerHideAnimations');
    let animations=localStorage.getItem('inj_animations_v790')!=='off';
    const syncAnim=()=>{document.body.classList.toggle('animations-off',!animations);anim?.classList.toggle('active',animations);anim?.setAttribute('aria-pressed',String(animations));};
    anim?.addEventListener('click',()=>{animations=!animations;localStorage.setItem('inj_animations_v790',animations?'on':'off');syncAnim()}); syncAnim();

    const live=document.getElementById('landscapeLive'), open=document.getElementById('drawerLandscapeLive'), close=document.getElementById('landscapeLiveExit');
    const openLive=()=>{if(!live)return;live.hidden=false;live.setAttribute('aria-hidden','false');document.body.classList.add('live-display-open');try{live.requestFullscreen?.()}catch(e){};requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')))};
    const closeLive=()=>{if(!live)return;live.hidden=true;live.setAttribute('aria-hidden','true');document.body.classList.remove('live-display-open');if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});};
    open?.addEventListener('click',openLive); close?.addEventListener('click',closeLive); document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!live?.hidden)closeLive()});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();

/* v7.9.0 final polish — subtle directional tint on the card that actually changed */
(() => {
  const originalRollValue = rollValue;
  window.rollValue = function(id, formatted, numericValue){
    const el=document.getElementById(id);
    const before=el ? Number(el.dataset.rollValue) : NaN;
    originalRollValue(id,formatted,numericValue);
    if(!el || document.body.classList.contains('privacy')) return;
    const after=Number(numericValue);
    if(!Number.isFinite(before)||!Number.isFinite(after)||before===after) return;
    const card=el.closest('.card'); if(!card)return;
    card.classList.remove('value-direction-up','value-direction-down');
    void card.offsetWidth;
    card.classList.add(after>before?'value-direction-up':'value-direction-down');
    clearTimeout(card._directionTintTimer);
    card._directionTintTimer=setTimeout(()=>card.classList.remove('value-direction-up','value-direction-down'),1150);
  };
})();

/* v7.9.1 — cycle theme, wallet-only mode and live display data sync */
(() => {
  const THEMES=['black','light','blue'];
  const LABELS={black:'Theme: Black',light:'Theme: Light',blue:'Theme: Dark Blue'};
  const ICONS={black:'●',light:'○',blue:'●'};
  const KEY='inj_theme_v790';
  function currentTheme(){
    const cls=[...document.body.classList].find(c=>c.startsWith('theme-'));
    return cls?cls.replace('theme-',''):(localStorage.getItem(KEY)||'black');
  }
  function setTheme(theme){
    ...
    localStorage.setItem('inj_theme',theme==='light'?'light':'dark');
    const btn=document.getElementById('drawerThemeCycle');
    if(btn){
      btn.querySelector('.interface-icon').textContent=ICONS[theme];
      btn.querySelector('span:last-child').textContent=LABELS[theme];
      btn.setAttribute('aria-label',`${LABELS[theme]}. Premi per cambiare tema`);
    }
    const meta=document.querySelector('meta[name="theme-color"]');
    if(meta)meta.content=theme==='light'?'#f4f6f9':theme==='blue'?'#030a16':'#000000';
    requestAnimationFrame(()=>window.dispatchEvent(new Event('resize')));
  }
  function moneyLive(value){
    try{return typeof money==='function'?money(value):new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(value||0)}catch{return `$${Number(value||0).toFixed(2)}`}
  }
  function syncLiveDisplay(){
    const total=(Number(state?.available)||0)+(Number(state?.staked)||0)+(Number(state?.rewards)||0);
    const price=Number(state?.price)||0;
    const set=(id,value)=>{const el=document.getElementById(id);if(el&&el.textContent!==value)el.textContent=value};
    set('landscapeMarketPrice',moneyLive(price));
    const change=Number(state?.change);
    set('landscapeMarketChange',Number.isFinite(change)?`${change>=0?'+':''}${change.toFixed(2)}%`:'—');
    const changeEl=document.getElementById('landscapeMarketChange');
    if(changeEl){changeEl.classList.toggle('positive',change>=0);changeEl.classList.toggle('negative',change<0)}
    set('landscapeWalletWorth',moneyLive(total*price));
    set('landscapeWalletInj',`${total.toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4})} INJ`);
    set('landscapeWalletAvailable',Number(state?.available||0).toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4}));
    set('landscapeWalletStaked',Number(state?.staked||0).toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4}));
    set('landscapeWalletRewards',Number(state?.rewards||0).toLocaleString('en-US',{minimumFractionDigits:6,maximumFractionDigits:6}));
  }
  function init(){
    setTheme(currentTheme());
    const themeBtn=document.getElementById('drawerThemeCycle');
    themeBtn?.addEventListener('click',()=>{
      const now=currentTheme();
      setTheme(THEMES[(THEMES.indexOf(now)+1)%THEMES.length]);
    });
    const open=document.getElementById('drawerLandscapeLive');
    open?.addEventListener('click',()=>setTimeout(syncLiveDisplay,0));
    syncLiveDisplay();
    setInterval(syncLiveDisplay,1000);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();

/* v7.9.2 — stable live percentage, directional digits and restored elegant Live Display */
(() => {
  try { ROLLING_IDS.delete('marketChange'); ROLLING_IDS.delete('priceChange'); } catch (_) {}

  function paintSignedChange(id, value) {
    const el = document.getElementById(id);
    if (!el) return;
    const n = Number(value);
    const text = Number.isFinite(n) ? `${n >= 0 ? '+' : ''}${n.toFixed(2)}%` : '—';
    if (el.textContent !== text) el.textContent = text;
    el.classList.toggle('positive', Number.isFinite(n) && n >= 0);
    el.classList.toggle('negative', Number.isFinite(n) && n < 0);
    el.setAttribute('aria-label', Number.isFinite(n) ? `Variazione 24 ore ${text}` : 'Variazione non disponibile');
  }

  function refreshStableChanges() {
    paintSignedChange('marketChange', state?.change);
    paintSignedChange('priceChange', state?.change);
    paintSignedChange('landscapeMarketChange', state?.change);
  }

  function syncElegantLiveDisplay() {
    const total = (Number(state?.available) || 0) + (Number(state?.staked) || 0) + (Number(state?.rewards) || 0);
    const price = Number(state?.price) || 0;
    ...
    const cash = value => {
      try { return typeof money === 'function' ? money(value) : new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(value || 0); }
      catch (_) { return `$${Number(value || 0).toFixed(2)}`; }
    };
    write('landscapeMarketPrice', cash(price));
    paintSignedChange('landscapeMarketChange', state?.change);
    write('landscapeWalletWorth', cash(total * price));
    write('landscapeWalletInj', `${total.toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4})} INJ`);
    write('landscapeWalletAvailable', Number(state?.available || 0).toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4}));
    write('landscapeWalletStaked', Number(state?.staked || 0).toLocaleString('en-US',{minimumFractionDigits:4,maximumFractionDigits:4}));
    write('landscapeWalletRewards', Number(state?.rewards || 0).toLocaleString('en-US',{minimumFractionDigits:6,maximumFractionDigits:6}));
  }

  function initV792() {
    refreshStableChanges();
    syncElegantLiveDisplay();
    document.getElementById('drawerLandscapeLive')?.addEventListener('click', () => requestAnimationFrame(syncElegantLiveDisplay));
    setInterval(() => { refreshStableChanges(); syncElegantLiveDisplay(); }, 750);
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initV792, {once:true});
  else initV792();
})();

/* v7.9.4 — remove wallet 24h card and keep Live Display staking/rewards synced */
(() => {
  function syncLiveDisplay794(){
    const staked=Number(state?.staked||0), rewards=Number(state?.rewards||0), available=Number(state?.available||0);
    const total=available+staked+rewards, price=Number(state?.price||0);
    const set=(id,value)=>{const el=document.getElementById(id);if(el&&el.textContent!==value)el.textContent=value};
    const fmt=(v,d)=>Number(v||0).toLocaleString('en-US',{minimumFractionDigits:d,maximumFractionDigits:d});
    try{set('landscapeMarketPrice',typeof money==='function'?money(price):`$${price.toFixed(4)}`)}catch{set('landscapeMarketPrice',`$${price.toFixed(4)}`)}
    try{set('landscapeWalletWorth',typeof money==='function'?money(total*price):`$${(total*price).toFixed(2)}`)}catch{set('landscapeWalletWorth',`$${(total*price).toFixed(2)}`)}
    set('landscapeWalletInj',`${fmt(total,4)} INJ`);
    set('landscapeWalletAvailable',fmt(available,4));
    set('landscapeWalletStaked',fmt(staked,4));
    set('landscapeWalletRewards',fmt(rewards,6));
  }
  const oldRefresh=window.refreshAll;
  if(typeof oldRefresh==='function')window.refreshAll=async function(...args){const r=await oldRefresh.apply(this,args);syncLiveDisplay794();return r};
  document.addEventListener('DOMContentLoaded',()=>{document.getElementById('walletCycleCard')?.remove();setTimeout(syncLiveDisplay794,300);setInterval(syncLiveDisplay794,1000)});
})();
