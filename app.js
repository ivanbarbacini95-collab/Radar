...
async function loadMarket(){
  ...
  (async()=>{
    ...
    if(number(market?.market_cap)>0){
      ...
      storage.setJSON('inj_market_metadata_v837',{
      ...
      const cached=storage.getJSON('inj_market_metadata_v837',{});
...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    storage.set('inj_address',address); setText('lastUpdate',`Updated ${new Date().toLocaleTimeString('en-US')}`);
...
function renderRewardHistory(error=''){
  ...
  const mhours=$('loadMoreWithdrawals'); if(mhours){ mhours.hidden=!state.rewardHistoryNextKey; mhours.disabled=state.rewardHistoryLoading; }
  ...
  host.innerHTML=rows.map(row=>{
    const explhoursr=row.hash?`https://explorer.injective.network/transaction/${encodeURIComponent(row.hash)}`:'';
...
function initPriceSimulator(){
  ...
  renderNetworkTokenomics();
  if(cached>0){ state.totalSupply=cached; setText('liveTotalSupply',`${cached.toLocaleString('en-US',{maximumFractionDigits:2})} INJ`); const updated=number(storage.get('inj_live_total_supply_updated','0')); setText('supplyUpdateTime',updated?new Date(updated).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}):'—'); }
  renderPriceSimulator();
  loadLiveTotalSupply();
